...
async function compareData() {
  ...
  const goliothDevice = goliothData.list?.find(
    (d) => d.id === GOLIOTH_DEVICE_ID
  )
  ...
  const comparisons = [
    ...
    {
      field: 'id (Golioth) vs external_device_id (DB)',
      golioth: goliothDevice?.id,
      db: dbDevice?.external_device_id,
    },
    { field: 'status', golioth: goliothDevice?.status, db: dbDevice?.status },
    {
      field: 'hardwareIds',
      golioth: goliothDevice?.hardwareIds || goliothDevice?.hardwareId,
      db: dbDevice?.hardware_ids,
    },
    {
      field: 'cohortId',
      golioth: goliothDevice?.cohortId,
      db: dbDevice?.cohort_id,
    },
    {
      field: 'lastSeenOnline',
      golioth: goliothDevice?.lastSeenOnline,
      db: dbDevice?.last_seen_online,
    },
    {
      field: 'lastSeenOffline',
      golioth: goliothDevice?.lastSeenOffline,
      db: dbDevice?.last_seen_offline,
    },
    {
      field: 'metadata',
      golioth: goliothDevice?.metadata,
      db: dbDevice?.metadata,
    },
    {
      field: 'tags',
      golioth: goliothDevice?.tags,
      db: 'N/A (not in DB schema)',
    },
  ]
