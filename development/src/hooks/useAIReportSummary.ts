...
interface ReportData {
  reportType: 'alert_history' | 'telemetry_trends' | 'audit_log' | 'device_health'
  dateRange: string
  totalRecords: number
...
export function useAIReportSummary() {
  ...
  const generateSummary = useCallback(async (reportData: ReportData): Promise<AIReportSummary | null> => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/ai-report-summary`, {
        method: 'POST',
        headers: {
      ...
      if (result.fallback || result.error) {
        console.warn('⚠️ AI summary unavailable, using rule-based:', result.error)
...
function generateRuleBasedSummary(reportData: ReportData): AIReportSummary {
  ...
  switch (reportType) {
    case 'alert_history': {
      ...
      const resolvedPercent = totalRecords > 0 ? ((totalRecords - unresolvedCount) / totalRecords * 100).toFixed(1) : '0'
      ...
      if (criticalCount > totalRecords * 0.2) {
        summary.redFlags.push(`High critical alert rate (${criticalCount}/${totalRecords}) - investigate root causes`)
      }

      if (unresolvedCount > totalRecords * 0.3) {
        summary.redFlags.push(`${unresolvedCount} unresolved alerts need attention`)
      }
    ...
    case 'audit_log': {
      ...
      if (failedCount > 0) {
        summary.redFlags.push(`${failedCount} failed actions require investigation`)
      }
