...
const LOCATION_CONTEXTS: Record<string, LocationContext> = {
  ...
  'walk-in cooler': {
    ...
    humidityMax: 95,
    context:
      'Walk-in coolers require 32-40°F (0-4°C) and 85-95% humidity for fresh produce storage.',
  },
  cooler: {
    tempMin: 0,
    tempMax: 4,
    ...
    humidityMax: 95,
    context:
      'Coolers should maintain refrigeration temperatures around 32-40°F (0-4°C).',
  },
  refrigerator: {
    tempMin: 1,
    tempMax: 4,
    ...
    humidityMax: 75,
    context:
      'Refrigerators should stay between 34-40°F (1-4°C) for food safety.',
  },
  freezer: {
    tempMin: -23,
    tempMax: -18,
    ...
    humidityMax: 10,
    context:
      'Freezers must maintain -10 to 0°F (-23 to -18°C) for proper food preservation.',
  },
  ...
  kitchen: {
    ...
    humidityMax: 60,
    context:
      'Commercial kitchens should maintain 65-75°F (18-24°C) with moderate humidity.',
  },
  'prep area': {
    tempMin: 18,
    tempMax: 24,
    ...
    humidityMax: 60,
    context:
      'Food prep areas need controlled temperatures around 65-75°F (18-24°C).',
  },
  'bean room': {
    tempMin: 18,
    tempMax: 22,
    ...
    humidityMax: 70,
    context:
      'Coffee bean storage requires cool, dry conditions around 65-72°F (18-22°C) with 50-70% humidity.',
  },
  ...
  produce: {
    ...
    humidityMax: 95,
    context:
      'Fresh vegetables need 45-55°F (7-13°C) and 85-95% humidity to prevent wilting.',
  },
  vegetable: {
    tempMin: 7,
    tempMax: 13,
    ...
    humidityMax: 95,
    context:
      'Vegetable storage requires cool temperatures and high humidity to maintain freshness.',
  },
  ...
  bathroom: {
    ...
    humidityMax: 70,
    context:
      'Bathrooms typically have higher humidity (40-70%) and should stay between 68-78°F (20-26°C).',
  },
  shower: {
    tempMin: 20,
    tempMax: 26,
    ...
    humidityMax: 80,
    context:
      'Shower areas have elevated humidity and should monitor for excess moisture above 80%.',
  },
  ...
  basement: {
    ...
    humidityMax: 50,
    context:
      'Basements should stay cool and dry, around 60-70°F (15-21°C) with 30-50% humidity to prevent mold.',
  },
  storage: {
    tempMin: 15,
    tempMax: 24,
    ...
    humidityMax: 60,
    context:
      'General storage areas benefit from stable conditions around 60-75°F (15-24°C).',
  },
  warehouse: {
    tempMin: 15,
    tempMax: 27,
    ...
    humidityMax: 60,
    context:
      'Warehouse spaces should maintain 60-80°F (15-27°C) depending on stored goods.',
  },
  ...
  'server room': {
    ...
    humidityMax: 60,
    context:
      'Server rooms require 64-80°F (18-27°C) and 40-60% humidity for optimal equipment performance.',
  },
  office: {
    tempMin: 20,
    tempMax: 24,
    ...
    humidityMax: 60,
    context:
      'Office spaces should maintain comfortable conditions around 68-75°F (20-24°C).',
  },
  lab: {
    tempMin: 20,
    tempMax: 24,
    ...
    humidityMax: 60,
    context:
      'Laboratory environments need stable conditions around 68-75°F (20-24°C) with controlled humidity.',
  },
}
...
export function StatisticalSummaryCard({
  ...
  console.log(
    '🌡️ [StatisticalSummaryCard] Rendering with temperatureUnit:',
    temperatureUnit
  )
  ...
  const sensorAnalyses = useMemo<SensorAnalysis[]>(() => {
    ...
    return Object.entries(sensorGroups)
      .map(([sensorKey, readings]) => {
        ...
        const typeId =
          sensorKey.startsWith('type_') && parts[1] ? parseInt(parts[1]) : null

        // Safely access readings array
        const firstReading = readings[0]
        const sensorName =
          (typeId && SENSOR_LABELS[typeId]) ||
    ...
    const fetchAIInsights = async () => {
      ...
      try {
        ...
        if (data.fallback || data.error) {
          console.warn(
            '⚠️ AI insights unavailable, using rule-based fallback:',
            data.error
          )
          if (mounted) {
            setAiInsights(generateRuleBasedInsights())
  ...
  const generateRuleBasedInsights = useCallback((): AIInsight[] => {
    console.log(
      '🤖 [aiInsights] Generating rule-based insights with temperatureUnit:',
      temperatureUnit
    )
    ...
    if (!hasWarning && !hasCritical) {
      insights.push({
        ...
        message:
          'Based on recent sensor data, all measurements are within normal operating ranges. Equipment is functioning properly.',
      })
    }
    ...
    console.log(
      '🤖 [generateRuleBasedInsights] Generated',
      insights.length,
      'insights'
    )
    return insights
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            title={
              useOpenAI ? 'Using OpenAI GPT-3.5' : 'Using rule-based analysis'
            }
          >
            {useOpenAI ? '🤖 AI' : '📊 Rules'}
          </button>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                      ...
                      <p className="font-semibold">
                        {formatValue(sensor.avg, sensor.sensorName)}
                      </p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Min</p>
                      <p className="font-semibold">
                        {formatValue(sensor.min, sensor.sensorName)}
                      </p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Max</p>
                      <p className="font-semibold">
                        {formatValue(sensor.max, sensor.sensorName)}
                      </p>
                    </div>
                  </div>
            ...
            <div className="space-y-2 pr-3">
              {aiInsights.map((insight, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`rounded-lg border p-3 ${getInsightBg(insight.type)}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant={getInsightColor(insight.type)}
                            className="text-xs"
                          >
                            {insight.type}
                          </Badge>
