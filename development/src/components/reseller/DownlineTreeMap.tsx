...
function TreeNodeRow({
  ...
  const tierBadge =
    TIER_BG_COLORS[node.tier_name ?? ''] ?? 'bg-gray-100 text-gray-700'
...
export function DownlineTreeMap({
  rootOrgId,
  className,
}: DownlineTreeMapProps) {
  ...
  const fetchTree = useCallback(async () => {
    ...
    try {
      ...
      const { data: rawTree } = await db.rpc('get_reseller_tree', {
      ...
      const orgIds = tree
        .map((n: ResellerTreeNode) => n.id)
        .filter((id: string) => id !== rootOrgId)
      ...
      for (const t of tierNames ?? [])
        tierMap[t.id] = { name: t.name, discount_pct: t.discount_pct }

      const countMap: Record<string, number> = {}
      const tierIdMap: Record<string, string> = {}
      for (const sc of sensorCounts ?? []) {
        countMap[sc.organization_id] = sc.effective_total
        if (sc.current_tier_id)
          tierIdMap[sc.organization_id] = sc.current_tier_id
      }

      const enriched: ResellerTreeNode[] = (
        tree as unknown as ResellerTreeNode[]
      )
        ...
        .map((n) => ({
          ...
          tier_name:
            tierIdMap[n.id] && tierMap[tierIdMap[n.id] as string]
              ? tierMap[tierIdMap[n.id] as string]?.name
              : undefined,
          tier_discount:
            tierIdMap[n.id] && tierMap[tierIdMap[n.id] as string]
              ? tierMap[tierIdMap[n.id] as string]?.discount_pct
              : undefined,
        }))
  ...
  if (loading) {
    return (
      <div
        className={cn(
          'flex items-center justify-center rounded-xl border border-white/[0.08] bg-gray-900/60 p-8',
          className
        )}
      >
  ...
  return (
    <div
      className={cn(
        'rounded-xl border border-white/[0.08] bg-gray-900/60',
        className
      )}
    >
      {/* Stats header */}
