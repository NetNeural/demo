...
export function AddMemberDialog({
  ...
  const handleAddMember = async () => {
    ...
    if (!emailRegex.test(trimmedEmail)) {
      console.log(
        '❌ Validation failed: invalid email format, raw value:',
    ...
    try {
      ...
      if (needsUserCreation && trimmedName) {
        ...
        if (!createResponse.success) {
          ...
          const errorObj = createResponse.error as
            | { message?: string }
            | string
            | undefined
          const errorMsg =
          ...
          if (userExists) {
            ...
            const addResponse = await edgeFunctions.members.add(
              organizationId,
              {
                email: trimmedEmail,
                role,
            ...
            if (!addResponse.success) {
              ...
              const addErrorObj = addResponse.error as
                | { message?: string }
                | string
                | undefined
              const addErrorMsg =
                typeof addErrorObj === 'string'
                  ? addErrorObj.toLowerCase()
                  : (addErrorObj?.message || '').toLowerCase()
              const alreadyMember =
                addErrorMsg.includes('already a member') ||
                addErrorMsg.includes('already exists')
              ...
              throw new Error(
                addErrorMsg || 'User exists but failed to add to organization'
              )
            }
        ...
        console.log(
          'ℹ️ User was automatically added to organization during creation'
        )
      ...
      if (addResponse.success) {
        ...
        const errorObj = addResponse.error as
          | { message?: string }
          | string
          | undefined
        const errorMsg =
        ...
        {showPasswordSuccess ? (
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                {fullName} has been added to the organization. Share these
            ...
            <div className="space-y-4 py-4">
              ...
              <Alert>
                <AlertDescription className="text-xs">
                  <strong>Important:</strong> The user will be required to
                  change this password on their first login. Make sure to share
              ...
              {needsUserCreation && (
                <>
                  <Alert>
                    ...
                    <AlertDescription>
                      No account found for <strong>{email}</strong>. We&apos;ll
                      create one for them.
                    </AlertDescription>
                  </Alert>
              ...
              {needsUserCreation && (
                <Alert>
                  <AlertDescription className="text-xs">
                    ...
                    login. You&apos;ll see the password after creating the
                    account.
                  </AlertDescription>
                </Alert>
              )}
