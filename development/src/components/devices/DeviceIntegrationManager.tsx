...
export function DeviceIntegrationManager() {
  ...
  const [integrationToConfig, setIntegrationToConfig] =
    useState<DeviceIntegration | null>(null)
      ...
      setIntegrations([
        {
          ...
          device_count:
            deviceData.devices?.filter(
              (d: LocalDevice) => d.isExternallyManaged
            ).length || 0,
        },
      ])
    } catch (error) {
  ...
  const handleMapDevice = async () => {
    if (!selectedDevice || !selectedIntegration || !externalDeviceId) {
      setMessage({
        type: 'error',
        text: 'Please select device, integration, and enter external device ID',
      })
      return
    }
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: 'Device successfully mapped to external system',
      })
      ...
      setMessage({
        type: 'error',
        text: 'Failed to map device to external system',
      })
    } finally {
      setLoading(false)
    }
  ...
  const handleUnmapDevice = async (deviceId: string) => {
    try {
      ...
      setMessage({
        type: 'success',
        text: 'Device unmapped from external system',
      })
      loadData() // Refresh data
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Connect a local device to an external IoT platform for unified
            management
          </CardDescription>
        </CardHeader>
      ...
      {integrationToConfig?.integration_type === 'mqtt' &&
        currentOrganization && (
          <MqttConfigDialog
