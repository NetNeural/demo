...
const PROTOCOL_INFO = {
  coap: {
    name: 'CoAP',
    description:
      'Constrained Application Protocol - optimized for cellular IoT',
    benefits: [
      'Ultra low bandwidth',
      'Low power consumption',
      'Native Nordic SDK support',
      'CBOR encoding',
      'Observe pattern',
    ],
    defaultPort: 5684,
  ...
  mqtt: {
    name: 'MQTT',
    description:
      'Message Queuing Telemetry Transport - reliable pub/sub messaging',
    benefits: [
...
export function NetNeuralLinkConfigDialog({
    ...
    if (savedConfig) {
      try {
        ...
        setConfig({
          ...
          device_routing:
            parsedConfig.device_routing || DEFAULT_CONFIG.device_routing,
          global_settings:
            parsedConfig.global_settings || DEFAULT_CONFIG.global_settings,
        })
      } catch (error) {
        console.warn('Failed to parse saved NetNeural-Link config:', error)
      }
    }
  }, [])
          ...
          if (response.success && response.data) {
            ...
            if (existingIntegration && existingIntegration.settings) {
              setConfig({
                ...
                protocols:
                  existingIntegration.settings.protocols ||
                  DEFAULT_CONFIG.protocols,
                device_routing:
                  existingIntegration.settings.device_routing ||
                  DEFAULT_CONFIG.device_routing,
                global_settings:
                  existingIntegration.settings.global_settings ||
                  DEFAULT_CONFIG.global_settings,
              })
            }
          }
        } catch (error) {
          console.warn('Failed to load existing NetNeural-Link config:', error)
        }
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Failed to save NetNeural-Link config:', error)
    } finally {
      setIsSaving(false)
    }
  }
    ...
    <>
      <div className="space-y-6">
        ...
        <Alert>
          ...
          <AlertDescription>
            <strong>NetNeural-Link:</strong> Auto-discovers devices via CoAP,
            ...
            <Tabs defaultValue="coap" className="w-full">
              ...
              {Object.entries(PROTOCOL_INFO).map(([protocolKey, info]) => {
                const protocolConfig =
                  config.protocols[protocolKey as keyof typeof config.protocols]
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Switch
                            ...
                            onCheckedChange={(enabled) =>
                              updateProtocolConfig(protocolKey, { enabled })
                            }
                          />
                        </div>
                      ...
                      {protocolConfig?.enabled && (
                        <CardContent className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <Label htmlFor={`${protocolKey}-endpoint`}>
                                Endpoint URL
                              </Label>
                              <Input
                            ...
                            <div>
                              <Label htmlFor={`${protocolKey}-auth`}>
                            ...
                            <div className="space-y-3 border-t pt-4">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div className="flex items-center justify-between">
                                  ...
                                  <Switch
                                    checked={
                                      protocolConfig?.options?.use_cbor || false
                                    }
                                    onCheckedChange={(enabled) =>
                                      updateProtocolConfig(protocolKey, {
                                        options: {
                            ...
                            <div className="space-y-3 border-t pt-4">
                              ...
                              <div className="grid grid-cols-3 gap-4">
                                <div>
                                  ...
                                  <Select
                                    value={String(
                                      protocolConfig?.options?.default_qos || 1
                                    )}
                                    onValueChange={(qos) =>
                                      updateProtocolConfig(protocolKey, {
                                        options: {
                                          ...protocolConfig?.options,
                                          default_qos: parseInt(qos) as
                                            | 0
                                            | 1
                                            | 2,
                                        },
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <Switch
                                    checked={
                                      protocolConfig?.options?.lwt_enabled ||
                                      false
                                    }
                                    onCheckedChange={(enabled) =>
                                      updateProtocolConfig(protocolKey, {
                            ...
                            <div className="space-y-3 border-t pt-4">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div>
                                  ...
                                  <Input
                                    value={
                                      protocolConfig?.options?.webhook_url || ''
                                    }
                                    onChange={(e) =>
                                      updateProtocolConfig(protocolKey, {
                                        options: {
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <Switch
                                    checked={
                                      protocolConfig?.options?.sse_enabled ||
                                      false
                                    }
                                    onCheckedChange={(enabled) =>
                                      updateProtocolConfig(protocolKey, {
                                  ...
                                  <Switch
                                    ...
                                    onCheckedChange={(enabled) =>
                                      updateProtocolConfig(protocolKey, {
                                        options: {
                                          ...protocolConfig?.options,
                                          require_delivery_receipt: enabled,
                                        },
                                      })
                                    }
                                  />
                          ...
                          {protocolConfig?.auth?.method !== 'none' && (
                            <Alert>
                              <Info className="h-4 w-4" />
                              <AlertDescription>
                                {protocolKey.toUpperCase()} authentication
                                credentials will be configured in production
                                deployment. Development uses demo credentials
                      ...
                      <CardContent className="space-y-6">
                        <div>
                          ...
                          <p className="mb-4 text-sm text-muted-foreground">
                            Click protocols to set preference order. Lower
                            numbers have higher priority.
                          </p>
                          <div className="flex flex-wrap gap-3">
                            {deviceType.protocols.map((protocol) => {
                              const isSelected =
                                routing.preferred_protocols.includes(protocol)
                              const priority =
                                routing.preferred_protocols.indexOf(protocol) +
                                1
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    const newProtocols = isSelected
                                      ? routing.preferred_protocols.filter(
                                          (p) => p !== protocol
                                        )
                                      : [
                                          ...routing.preferred_protocols,
                                          protocol,
                                        ]
                                    updateDeviceRouting(deviceType.id, {
                                      preferred_protocols: newProtocols,
                                    })
                                  }}
                                  className="relative"
                                >
                                  ...
                                  {isSelected && (
                                    <Badge className="ml-2 flex h-5 w-5 items-center justify-center text-xs">
                                      {priority}
                                    </Badge>
                                  )}
                                </Button>
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {routing.preferred_protocols.length > 0 ? (
                                routing.preferred_protocols.map(
                                  (protocol, index) => (
                                    <Badge
                                      key={protocol}
                                      variant="secondary"
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Enter a name for this NetNeural-Link integration"
                    required
                  />
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          checked={
                            config.global_settings.device_discovery_enabled
                          }
                          onCheckedChange={(enabled) =>
                            setConfig((prev) => ({
                              ...prev,
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Automatically detect device capabilities from
                            metadata
                          </p>
                        </div>
                        <Switch
                          checked={
                            config.global_settings.auto_capability_detection
                          }
                          onCheckedChange={(enabled) =>
                            setConfig((prev) => ({
                              ...prev,
        ...
        <div className="mb-6 flex items-center gap-2">
          ...
          <div>
            <h2 className="text-3xl font-bold tracking-tight">
              {integrationId ? 'Edit' : 'Configure'} NetNeural-Link
            </h2>
