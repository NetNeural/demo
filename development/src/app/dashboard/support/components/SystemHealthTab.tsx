  ...
  const fetchEdgeMetrics = useCallback(async () => {
    try {
      ...
      let query = supabase
        ...
        .select(
          'activity_type, status, response_time_ms, created_at, error_message'
        )
      ...
      for (const entry of entries) {
        ...
        if (!byType[key])
          byType[key] = { total: 0, success: 0, failed: 0, avgMs: 0 }
        byType[key].total++
      ...
      for (const key of Object.keys(byType)) {
        const typeEntries = entries.filter(
          (e) => e.activity_type === key && e.response_time_ms != null
        )
        byType[key]!.avgMs =
          typeEntries.length > 0
            ? Math.round(
                typeEntries
                  .map((e) => e.response_time_ms!)
                  .reduce((a, b) => a + b, 0) / typeEntries.length
              )
            : 0
      }
  ...
  const fetchErrorMetrics = useCallback(async () => {
    try {
      ...
      let efTotalQuery = supabase
        ...
        .gte('created_at', twentyFourHoursAgo)
      if (!isSuperAdmin)
        efTotalQuery = efTotalQuery.eq('organization_id', organizationId)
      ...
      const metrics: ErrorMetric[] = [
        {
          ...
          percentage:
            (efTotal || 0) > 0
              ? Math.round(((efErrors || 0) / (efTotal || 1)) * 100)
              : 0,
        },
        {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={refreshAll}>
            <RefreshCw className="mr-1 h-4 w-4" /> Refresh Now
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {edgeMetrics?.totalInvocations ?? 0}
                </p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {edgeMetrics?.avgResponseTime ?? 0}ms
                </p>
                <p className="text-sm text-muted-foreground">
                  Avg Response Time
                </p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              <AlertTriangle
                className={`h-8 w-8 ${getHealthColor(edgeMetrics?.errorRate ?? 0)}`}
              />
              <div>
                <p className="text-2xl font-bold">
                  {edgeMetrics?.errorRate ?? 0}%
                </p>
                <p className="text-sm text-muted-foreground">Error Rate</p>
                <Badge
                  variant={getHealthBadge(edgeMetrics?.errorRate ?? 0).variant}
                ...
                .map(([type, stats]) => {
                  ...
                  return (
                    <div key={type} className="flex items-center gap-4">
                      ...
                      <div className="flex min-w-[200px] items-center justify-end gap-3 text-xs text-muted-foreground">
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8">
              {edgeMetrics.hourlyTrend.map((h) => (
                <div key={h.hour} className="rounded border p-2 text-center">
                  <p className="font-mono text-xs font-medium">{h.hour}</p>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="rounded-lg border p-4 text-center">
                <p className="text-3xl font-bold">
                  {storageMetrics.telemetryCount.toLocaleString()}
                </p>
                <p className="text-sm text-muted-foreground">
                  Telemetry Records
                </p>
              ...
              <div className="rounded-lg border p-4 text-center">
                <p className="text-3xl font-bold">
                  {storageMetrics.alertCount.toLocaleString()}
                </p>
                <p className="text-sm text-muted-foreground">Alert Records</p>
              </div>
              <div className="rounded-lg border p-4 text-center">
                <p className="text-3xl font-bold">
                  {storageMetrics.auditLogCount.toLocaleString()}
                </p>
                <p className="text-sm text-muted-foreground">
                  Audit Log Entries
                </p>
              ...
              {errorMetrics.map((metric) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {metric.percentage > 0 && (
                      <Badge
                        variant={getHealthBadge(metric.percentage).variant}
                        className="text-xs"
                      >
