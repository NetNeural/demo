...
export default function AdminToolsTab({ organizationId }: Props) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      switch (exportType) {
        case 'telemetry': {
          const { data: telemetry, error } = await supabase
            ...
            .select('device_id, telemetry, received_at, device_timestamp, organization_id')
      ...
      toast.success(`Exported ${data.length} records as ${exportFormat.toUpperCase()}`)
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Export organization data as CSV or JSON (up to 5,000 records)</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Batch administrative actions (use with caution)</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 border rounded-lg">
              <div>
                <p className="font-medium text-sm">Archive Old Alerts</p>
