...
const METRICS = [
  { key: 'temperature', label: 'Temperature', icon: ThermometerSun, unit: '°C' },
...
export function AIForecastingSection({ organizationId, timeRange }: AIForecastingSectionProps) {
  ...
  const metricForecasts = useMemo((): MetricForecast[] => {
    ...
    return METRICS.map(({ key, label, icon, unit }) => {
      ...
      if (points.length === 0) {
        return {
          metric: key, label, icon, unit,
          currentAvg: 0, predictedAvg: 0,
          trend: 'stable' as const, trendPercent: 0,
          confidence: 0, anomalyCount: 0, anomalyDevices: [],
        };
      }
      ...
      const recentValues = points.slice(Math.floor(recentCutoff)).map(p => p.y);
      ...
      // Trend
      const changePercent = currentAvg !== 0 ? ((predictedAvg - currentAvg) / Math.abs(currentAvg)) * 100 : 0;
      ...
      const stdDev = Math.sqrt(allValues.reduce((s, v) => s + (v - mean) ** 2, 0) / allValues.length);
      ...
      return {
        metric: key, label, icon, unit,
        currentAvg, predictedAvg,
        trend, trendPercent: Math.abs(changePercent),
        confidence: r2,
        anomalyCount,
  ...
  const batteryPredictions = useMemo((): BatteryPrediction[] => {
    ...
    return predictions
      ...
      .sort((a, b) => (a.depletionHours ?? Infinity) - (b.depletionHours ?? Infinity));
            ...
            {metricForecasts.length === 0 ? (
              ...
              <div className="space-y-3">
                {metricForecasts.map((forecast) => {
                  ...
                  return (
                    <div key={forecast.metric} className="flex items-center justify-between p-3 rounded-lg border">
                      <div className="flex items-center gap-3">
                        <Icon className="w-5 h-5 text-muted-foreground flex-shrink-0" />
                        <div>
                          <p className="text-sm font-medium">{forecast.label}</p>
                          <p className="text-xs text-muted-foreground">
                            Now: {forecast.currentAvg.toFixed(1)}{forecast.unit}
                            {' → '}
                            Predicted: {forecast.predictedAvg.toFixed(1)}{forecast.unit}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {forecast.anomalyCount > 0 && (
                          <Badge variant="destructive" className="text-xs" title={`${forecast.anomalyCount} anomalous readings from ${forecast.anomalyDevices.length} device(s)`}>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className={`text-xs font-medium ${
                            forecast.trend === 'rising' ? 'text-green-600' :
                            forecast.trend === 'falling' ? 'text-red-600' : 'text-muted-foreground'
                          }`}>
                        ...
                        >
                          {forecast.confidence > 0.7 ? '●' : forecast.confidence > 0.4 ? '◐' : '○'}
                        </Badge>
                      </div>
                    </div>
                  );
                ...
                <p className="text-[10px] text-muted-foreground text-center pt-1">
                  ● high confidence · ◐ moderate · ○ low — based on linear regression (R²)
                </p>
              </div>
            )}
          </CardContent>
            ...
            {batteryPredictions.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[240px] overflow-y-auto">
                {batteryPredictions.slice(0, 8).map((pred) => {
                  const isUrgent = pred.depletionHours !== null && pred.depletionHours < 48;
                  const isCritical = pred.depletionHours !== null && pred.depletionHours < 12;
                  return (
                    <div
                      key={pred.deviceId}
                      className={`flex items-center justify-between p-3 rounded-lg border ${
                        isCritical ? 'border-red-200 bg-red-50/50 dark:bg-red-950/20' :
                        isUrgent ? 'border-yellow-200 bg-yellow-50/50 dark:bg-yellow-950/20' : ''
                      }`}
                    >
                      <div className="min-w-0">
                        <p className="text-sm font-medium truncate">{pred.deviceName}</p>
                        <p className="text-xs text-muted-foreground">
                          Currently {pred.currentBattery.toFixed(0)}% · draining {Math.abs(pred.ratePerHour).toFixed(2)}%/hr
                        </p>
                      </div>
                      <Badge
