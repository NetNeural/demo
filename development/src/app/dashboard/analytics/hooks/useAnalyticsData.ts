    ...
    async function fetchAnalytics() {
      try {
        ...
        const { data: devicesData, error: devicesError } = await supabase
          .from('devices')
          .select(
            'id, name, status, last_seen, serial_number, device_type, metadata, organization_id'
          )
        ...
        const { data: ackData } = await supabase
          ...
          .select(
            'alert_id, acknowledged_at, alerts!inner(created_at, resolved_at)'
          )
        ...
        const resolvedAlertsWithTime = alerts.filter(
          (a) => a.resolved_at && a.is_resolved && a.created_at
        )
        ...
        const devicePerformancePromises = devices.map(
          async (device): Promise<DevicePerformance> => {
            const { data: telemetryData, error: telemetryError } =
              await supabase
            ...
            if (telemetryError)
              console.error(
                `[Analytics] Telemetry error for ${device.name}:`,
                telemetryError
              )
            ...
            const batteryValues = telemetry
              .map((t) =>
                extractMetricValue(
                  t.telemetry as Record<string, unknown>,
                  'battery'
                )
              )
            ...
            const avgBattery =
              batteryValues.length > 0
                ? batteryValues.reduce((a, b) => a + b, 0) /
            ...
            const uptimePercentage = Math.min(
              100,
              (dataPointsCount / expectedDataPoints) * 100
            )
        ...
        const connectivityRate =
          totalDevices > 0
            ? Math.round((recentlyActive / totalDevices) * 100)
            : 0
        const errorRate =
          totalDevices > 0
            ? Math.round(((errorDevices + offlineDevices) / totalDevices) * 100)
            : 0
        const performanceScore = Math.round(
          overallHealth * 0.6 + connectivityRate * 0.3 + (100 - errorRate) * 0.1
        )
        ...
        const totalDataPoints = devicePerformance.reduce(
          (sum, d) => sum + d.data_points_count,
          0
        )
