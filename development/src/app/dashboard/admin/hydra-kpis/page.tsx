...
export default function HydraKPIDashboardPage() {
  ...
  const loadKPIs = async () => {
    ...
    try {
      ...
      const sensorCounts = results[0].data as
        | {
      ...
      const payoutsPending = results[4].data as
      ...
      const syncLog = results[7].data as {
        status: string
        orgs_processed: number
        tier_changes: number
        duration_ms: number
      ...
      const resellers = results[8].data as
      ...
      for (const sc of sensorCounts ?? []) {
        const tierName = sc.current_tier_id
          ? (tierMap[sc.current_tier_id] ?? 'Unknown')
          : 'Unassigned'
      ...
      const resellerDetails = (resellers ?? [])
        .map((r) => {
          const sc = (sensorCounts ?? []).find(
            (s) => s.organization_id === r.id
          )
          return {
            name: r.name,
            slug: r.slug,
      ...
      setKpis({
        ...
        total_payouts_pending: (payoutsPending ?? []).reduce(
          (s, p) => s + p.payout_amount,
          0
        ),
        total_payouts_paid: (payoutsPaid ?? []).reduce(
          (s, p) => s + p.payout_amount,
          0
        ),
        total_invites_pending: (invites ?? []).filter(
          (i) => i.status === 'pending'
        ).length,
        total_invites_accepted: (invites ?? []).filter(
          (i) => i.status === 'accepted'
        ).length,
  ...
  const triggerSync = async () => {
    ...
    try {
      ...
      const token = (await supabase.auth.getSession()).data.session
        ?.access_token
      await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/reseller-sensor-sync`,
        {
          method: 'POST',
          headers: {
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            onClick={triggerSync}
            disabled={syncing}
            className="bg-cyan-600 text-xs hover:bg-cyan-500"
          >
        ...
        {[
          {
            ...
            value: kpis.total_resellers,
            color: 'text-cyan-400',
          },
          {
            icon: Wifi,
            label: 'Total Sensors',
            value: kpis.total_sensors.toLocaleString(),
            color: 'text-emerald-400',
          },
          {
            icon: BarChart3,
            label: 'Avg per Org',
            value: kpis.avg_sensors_per_org.toLocaleString(),
            color: 'text-violet-400',
          },
          {
            icon: TrendingUp,
            label: 'Tier Changes (30d)',
            value: kpis.recent_tier_changes,
            color: 'text-amber-400',
          },
          {
            icon: DollarSign,
            label: 'Payouts Pending',
            value: `$${kpis.total_payouts_pending.toFixed(2)}`,
            color: 'text-cyan-400',
          },
          {
            icon: Activity,
            label: 'Grace Periods',
            value: kpis.pending_grace_periods,
            color: 'text-red-400',
          },
          ...
          <CardContent>
            <div className="space-y-3">
              {Object.entries(kpis.tier_distribution)
                .sort(([, a], [, b]) => b - a)
                .map(([tier, count]) => {
                  const total = Object.values(kpis.tier_distribution).reduce(
                    (a, b) => a + b,
                    0
                  )
                  ...
                  <TableRow key={r.slug} className="hover:bg-muted/30">
                    ...
                    <TableCell>
                      <Badge
                        className={cn(
                          'text-xs',
                          TIER_BG_COLORS[r.tier] ?? 'bg-gray-700 text-gray-300'
                        )}
                      >
                        {r.tier}
                      </Badge>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-3">
            {[
              {
                label: 'Invitations Sent (Pending)',
                value: kpis.total_invites_pending,
                color: 'text-amber-400',
              },
              {
                label: 'Invitations Accepted',
                value: kpis.total_invites_accepted,
                color: 'text-emerald-400',
              },
              {
                label: 'Acceptance Rate',
                value:
                  kpis.total_invites_pending + kpis.total_invites_accepted > 0
            ...
            {kpis.sync_log_last ? (
              <div className="space-y-3">
                {[
                  {
                    label: 'Status',
                    value: kpis.sync_log_last.status,
                    badge: true,
                  },
                  {
                    label: 'Orgs Processed',
                    value: kpis.sync_log_last.orgs_processed,
                  },
                  {
                    label: 'Tier Changes',
                    value: kpis.sync_log_last.tier_changes,
                  },
                  {
                    label: 'Duration',
                    value: `${kpis.sync_log_last.duration_ms}ms`,
                  },
                  {
                    label: 'Run At',
                    value: new Date(
                      kpis.sync_log_last.created_at
                    ).toLocaleString(),
                  },
