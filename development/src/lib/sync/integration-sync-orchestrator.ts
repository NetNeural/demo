...
export class IntegrationSyncOrchestrator {
  ...
  private async findMatchingDevice(
    ...
    const byExternalId = localDevices.find(
      (d) => d.external_device_id === externalDevice.id
    )
  ...
  private async updateDevice(
    ...
    await this.supabase.from('devices').update(updateData).eq('id', deviceId)
   ...
   */
  private async logFirmwareVersion(
    deviceId: string,
    version: string
  ): Promise<void> {
    ...
    if (device && device.firmware_version !== version) {
      await this.supabase.from('device_firmware_history').insert({
        device_id: deviceId,
