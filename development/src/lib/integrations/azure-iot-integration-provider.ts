...
export class AzureIotIntegrationProvider extends DeviceIntegrationProvider {
  ...
  override async getDevice(deviceId: string): Promise<DeviceData> {
    try {
      ...
      throw new Error(`Device ${deviceId} not found: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }
  ...
  override async getDeviceStatus(deviceId: string): Promise<DeviceStatus> {
    try {
      ...
      throw new Error(`Failed to get status for device ${deviceId}: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }
  ...
  override async updateDevice(deviceId: string, updates: DeviceUpdate): Promise<DeviceData> {
    try {
      ...
      throw new Error(`Failed to update device ${deviceId}: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }
  ...
  private async mapDeviceToGeneric(device: AzureDevice): Promise<DeviceData> {
    ...
    // Determine status
    const status = device.connectionState === 'Connected' ? 'online' : 
                   device.connectionState === 'Disconnected' ? 'offline' : 
                   'unknown';

    // Get last activity
    const lastSeen = device.lastActivityTime ? new Date(device.lastActivityTime) : undefined;
    ...
    return {
      ...
      lastSeen: lastSeen,
      createdAt: device.statusUpdatedTime ? new Date(device.statusUpdatedTime) : new Date(),
      updatedAt: device.connectionStateUpdatedTime ? new Date(device.connectionStateUpdatedTime) : new Date(),
    };
  }
  ...
  private mapTwinToStatus(twin: AzureTwin): DeviceStatus {
    ...
    const connectionState = twin.connectionState === 'Connected' ? 'online' :
                           twin.connectionState === 'Disconnected' ? 'offline' :
                           'unknown';
    ...
    const firmwareVersion = reported['$version'] as string || reported['firmwareVersion'] as string || 'unknown';
    const firmwareUpdateAvailable = !!desired['firmwareVersion'] && 
    ...
    const signalStrength = reported['signalStrength'] as number || reported['rssi'] as number;
