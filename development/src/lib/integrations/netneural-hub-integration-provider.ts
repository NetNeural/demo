...
export class NetNeuralHubIntegrationProvider extends DeviceIntegrationProvider {
  ...
  constructor(config: ProviderConfig) {
    ...
    this.integrationId = (config.credentials?.integrationId as string) || config.projectId || 'netneural-hub';
    ...
    const hubConfig = (config.credentials?.config as NetNeuralHubConfig) || this.getDefaultConfig();
  ...
  private setupMqttDiscovery(): void {
    ...
    this.mqttClient.on('message', (topic, message) => {
      try {
        ...
        console.error('[NetNeural Hub] Failed to parse discovery message:', error);
      }
    });
  }

  private handleDeviceDiscovery(protocol: string, topic: string, payload: Record<string, unknown>): void {
    ...
    console.log(`[NetNeural Hub] Device discovered via ${protocol}: ${deviceId}`);
  }

  private extractDeviceIdFromTopic(topic: string, payload: Record<string, unknown>): string | null {
    ...
    // Fallback to payload
    return (payload.device_id || payload.deviceId || payload.id) as string | null;
  }
  ...
  private detectCapabilities(payload: Record<string, unknown>): string[] {
    ...
    const sensorFields = ['temperature', 'humidity', 'pressure', 'light', 'motion', 'battery', 'rssi'];
  ...
  private async _testConnectionInternal(): Promise<TestConnectionResult> {
    ...
    if (this.config.protocols.mqtt?.enabled) {
      try {
        ...
        results.mqtt = false;
        errors.push(`MQTT error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    if (this.config.protocols.https?.enabled) {
      try {
        ...
        results.https = false;
        errors.push(`HTTPS error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    if (this.config.protocols.coap?.enabled) {
      ...
      errors.push('CoAP testing not yet implemented (requires coap npm package)');
    }

    const success = Object.values(results).some((r) => r === true);
    const message = success
      ? `Connected to ${Object.entries(results).filter(([, v]) => v).map(([k]) => k.toUpperCase()).join(', ')}`
  ...
  override async listDevices(options?: PaginationOptions): Promise<DeviceListResult> {
    const allDevices = Array.from(this.deviceCache.values()).map((cached) => cached.device);
  ...
  override async updateDevice(deviceId: string, updates: DeviceUpdate): Promise<DeviceData> {
    ...
    const updatedDevice: DeviceData = {
      ...
      metadata: updates.metadata ? { ...cached.device.metadata, ...updates.metadata } : cached.device.metadata,
   ...
   */
  override async queryTelemetry(query: TelemetryQuery): Promise<TelemetryData[]> {
