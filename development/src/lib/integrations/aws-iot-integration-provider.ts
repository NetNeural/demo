...
export class AwsIotIntegrationProvider extends DeviceIntegrationProvider {
  ...
  override async getDevice(deviceId: string): Promise<DeviceData> {
    try {
      ...
      throw new Error(`Failed to get AWS IoT device: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  override async updateDevice(deviceId: string, updates: DeviceUpdate): Promise<DeviceData> {
    try {
      ...
      throw new Error(`Failed to update AWS IoT device: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async mapThingToDevice(thing: AwsThing): Promise<DeviceData> {
    ...
    if (shadow?.metadata?.reported) {
      const timestamps = Object.values(shadow.metadata.reported).map(m => m.timestamp);
    ...
    return {
      ...
      hardwareIds: thing.attributes?.serialNumber ? [thing.attributes.serialNumber] : undefined,
  ...
  private mapShadowToStatus(shadow: AwsShadow): DeviceStatus {
    ...
    const connectionState: 'online' | 'offline' | 'unknown' = reported.connected === true ? 'online' : 
                                                             reported.connected === false ? 'offline' : 
                                                             'unknown';

    // Get last activity timestamp
    const lastActivity = shadow.timestamp ? new Date(shadow.timestamp * 1000) : new Date();
    ...
    const health = {
      battery: typeof reported.battery === 'number' ? reported.battery : undefined,
      signalStrength: typeof reported.rssi === 'number' ? reported.rssi : undefined,
      temperature: typeof reported.temperature === 'number' ? reported.temperature : undefined,
    };
