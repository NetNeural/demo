...
export class GoliothIntegrationProvider extends DeviceIntegrationProvider {
  ...
  constructor(config: ProviderConfig) {
    ...
    const integrationId =
      (config.credentials?.integrationId as string) || `golioth-${projectId}`
  ...
  async getDeviceStatus(deviceId: string): Promise<DeviceStatus> {
    ...
    const firmwareVersion = device.metadata?.firmware_version as
      | string
      | undefined
    const updateInfo = device.metadata?.update as
    ...
    const firmware = {
      ...
      components: updateInfo
        ? this.parseFirmwareComponents(updateInfo)
        : undefined,
    }
    ...
    return {
      ...
      lastActivity: new Date(
        device.lastSeenOnline || device.last_seen || device.updated_at
      ),
      firmware,
  ...
  async deployFirmware(
    ...
    try {
      ...
      throw new Error(
        `Failed to deploy firmware: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  private mapToGenericDevice(goliothDevice: GoliothDevice): DeviceData {
    ...
    const batteryLevel =
      (goliothDevice.metadata?.battery_level as number) || undefined
    const signalStrength =
      (goliothDevice.metadata?.signal_strength as number) || undefined
    ...
    const parentDeviceId =
      ...
      goliothDevice.tags
        ?.find((tag) => tag.startsWith('gateway:'))
        ?.replace('gateway:', '') ||
      goliothDevice.tags
    ...
    return {
      ...
      hardwareIds:
        goliothDevice.hardwareIds ||
        (goliothDevice.hardware_id ? [goliothDevice.hardware_id] : undefined),
      ...
      lastSeen: goliothDevice.lastSeenOnline
        ...
        : goliothDevice.last_seen
          ...
          : undefined,
      lastSeenOnline: goliothDevice.lastSeenOnline
        ? new Date(goliothDevice.lastSeenOnline)
        : undefined,
      lastSeenOffline: goliothDevice.lastSeenOffline
        ? new Date(goliothDevice.lastSeenOffline)
        : undefined,
