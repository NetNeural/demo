...
export class GoliothAPI {
  ...
  constructor(config?: {
    ...
    this.baseURL = config?.baseUrl || process.env.GOLIOTH_BASE_URL || 'https://api.golioth.io';
  ...
  async getProjects(): Promise<GoliothProject[]> {
    const response = await this.request<{ data: GoliothProject[] }>('/v1/projects');
  ...
  async getDevices(projectId?: string): Promise<GoliothDevice[]> {
    if (!this.enabled) {
      ...
      console.info('Golioth API not configured. Set GOLIOTH_API_KEY and GOLIOTH_PROJECT_ID to enable.');
      return [];
    }

    const id = projectId || this.projectId;
    const response = await this.request<{ data: GoliothDevice[] }>(`/v1/projects/${id}/devices`);
    return response.data;
  }

  async getDevice(deviceId: string, projectId?: string): Promise<GoliothDevice> {
    const id = projectId || this.projectId;
    return this.request<GoliothDevice>(`/v1/projects/${id}/devices/${deviceId}`);
  }

  async createDevice(device: Partial<GoliothDevice>, projectId?: string): Promise<GoliothDevice> {
    const id = projectId || this.projectId;
  ...
  async updateDevice(deviceId: string, updates: Partial<GoliothDevice>, projectId?: string): Promise<GoliothDevice> {
    const id = projectId || this.projectId;
    return this.request<GoliothDevice>(`/v1/projects/${id}/devices/${deviceId}`, {
      method: 'PATCH',
  ...
  async getDeviceState(deviceId: string, projectId?: string): Promise<GoliothDeviceState> {
    const id = projectId || this.projectId;
    return this.request<GoliothDeviceState>(`/v1/projects/${id}/devices/${deviceId}/state`);
  }

  async setDeviceState(deviceId: string, state: Record<string, unknown>, projectId?: string): Promise<void> {
    const id = projectId || this.projectId;
  ...
  async getDeviceStream(deviceId: string, options?: {
    start?: string;
    end?: string;
    limit?: number;
    path?: string;
  }, projectId?: string): Promise<GoliothStreamData[]> {
    const id = projectId || this.projectId;
  ...
  // Recent Activity
  async getRecentActivity(projectId?: string, limit: number = 10): Promise<Array<{
    id: string;
    type: 'device_connected' | 'device_disconnected' | 'data_received' | 'alert_triggered';
    message: string;
    timestamp: string;
    device_id?: string;
    device_name?: string;
      ...
      devices.forEach(device => {
        if (device.last_seen) {
          activities.push({
            ...
            type: device.status === 'online' ? 'device_connected' : 'device_disconnected' as const,
