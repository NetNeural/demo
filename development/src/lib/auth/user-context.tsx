  ...
  const fetchUserProfile = useCallback(
    async (userId: string) => {
      try {
        ...
        if (!profileData) {
          console.log(
            'User profile not found, creating new profile for:',
            userId
          )
        ...
        if (memberships.length === 0) {
          console.warn(
            'User has no organization memberships. Attempting to assign to default organization.'
          )
          ...
          try {
            ...
            if (defaultOrgError) {
              console.error(
                'Error querying default organization:',
                defaultOrgError
              )
              setError(
                'Failed to access default organization. Please contact your administrator.'
              )
              return
            }
            ...
            if (!defaultOrg) {
              console.error(
                'Default organization not found. Available organizations might be restricted by RLS policies.'
              )
              setError(
                'Default organization not found. Please contact your administrator to set up organization access.'
              )
              return
            }
            ...
            console.log(
              'Attempting to create membership with data:',
              membershipData
            )
            ...
            const { data: newMembership, error: membershipError } =
              await supabase
                .from('organization_members')
            ...
            if (membershipError) {
              ...
              setError(
                'Failed to assign organization access. Please contact your administrator.'
              )
              return
            }
            ...
            if (!newMembership) {
              console.error(
                'Organization membership creation succeeded but returned no data'
              )
              showNotification(
                'warning',
            ...
            setError(
              'Failed to set up organization access. Please contact your administrator.'
            )
            return
          }
        }
