...
describe('Admin Support Duty System', () => {
  it('shift has clock_in timestamp', () => {
    const shift = { id: 'shift-1', admin_user_id: 'admin-1', clocked_in_at: new Date().toISOString(), clocked_out_at: null }
  ...
  it('completed shift has both timestamps', () => {
    ...
    expect(new Date(shift.clocked_out_at) > new Date(shift.clocked_in_at)).toBe(true)
  })

  it('duty status shows admins online', () => {
    const dutyAdmins = [{ id: 'admin-1', clocked_in_at: new Date().toISOString() }]
