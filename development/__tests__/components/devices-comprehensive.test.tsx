...
describe('DevicesHeader', () => {
  ...
  test('renders header with title', () => {
    render(<DevicesHeader onNewDevice={mockOnNewDevice} onRefresh={mockOnRefresh} />)
...
describe('DevicesList', () => {
  ...
  test('handles click on device', async () => {
    ...
    render(<DevicesList devices={mockDevices} onDeviceClick={mockOnDeviceClick} />)
...
describe('TransferDeviceDialog', () => {
  ...
  test('displays organization options', () => {
    ...
    expect(screen.getByRole('combobox') || screen.getByRole('button', { name: /select|organization/i })).toBeInTheDocument()
  })
...
describe('DeviceIntegrationManager', () => {
  ...
  test('displays no integration state when device has no integration', () => {
    ...
    expect(screen.getByText(/no integration|not connected/i)).toBeInTheDocument()
  })

  test('displays connect button when no integration', () => {
    render(<DeviceIntegrationManager device={mockDevice} />)
    
    expect(screen.getByRole('button', { name: /connect|add integration/i })).toBeInTheDocument()
  })

  test('handles connect button click', async () => {
    render(<DeviceIntegrationManager device={mockDevice} />)
    
    const connectButton = screen.getByRole('button', { name: /connect|add integration/i })
    await userEvent.click(connectButton)
    
    // Dialog should open
    await waitFor(() => {
      expect(screen.getByRole('dialog') || screen.getByText(/select integration/i)).toBeInTheDocument()
    })
  })
})
