...
describe('Mercury AI Response Quality', () => {
  it('response is under 150 words', () => {
    const response =
      'To add a device, go to the Devices page and click Add Device. Enter the device name and select its type.'
  ...
  it('response is plain text, not markdown', () => {
    const response =
      'You can reset your password from the Settings page under Security.'
  ...
  it('off-topic questions are declined', () => {
    const offTopicResponse =
      'I can only help with questions about the NetNeural IoT platform.'
