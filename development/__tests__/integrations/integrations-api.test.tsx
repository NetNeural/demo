...
describe('Integrations API - Full Stack Tests', () => {
  ...
  let mockSession: { access_token: string; user: { id: string; email: string; role: string } };
  ...
  describe('Database Schema Validation', () => {
    test('device_integrations table has correct structure', async () => {
      ...
      const { error } = await supabase
        ...
        .select('id, organization_id, integration_type, name, settings, status, created_at, updated_at')
        .limit(1);
  ...
  describe('GET /integrations - List Integrations', () => {
    test('returns 401 without authentication', async () => {
      ...
      const response = await fetch(`${SUPABASE_URL}/functions/v1/integrations?organization_id=${testOrganizationId}`, {
        method: 'GET',
        headers: {
    ...
    test('filters by integration type when specified', async () => {
      const mockData = {
        integrations: [
          { id: '1', type: 'golioth', name: 'Test Golioth', status: 'active', deviceCount: 5, settings: {} }
        ],
        count: 1,
    ...
    test('includes device count in response', async () => {
      const mockData = {
        integrations: [
          { id: '1', type: 'golioth', name: 'Test', status: 'active', deviceCount: 10, settings: {} }
        ],
        count: 1
      };
  ...
  describe('PUT /integrations - Update Integration', () => {
    ...
    test('updates integration settings', async () => {
      ...
      if (response.ok) {
        const data = await response.json();
        expect(data.integration.settings).toHaveProperty('apiKey', 'updated-api-key');
...
describe('Integration Frontend Component Tests', () => {
  ...
  test('maps integration status to UI badge variants', () => {
    const getStatusBadge = (status: string) => {
      switch (status) {
        case 'active': return { color: 'green', text: 'Active' };
        case 'inactive': return { color: 'secondary', text: 'Inactive' };
        case 'error': return { color: 'red', text: 'Error' };
        case 'not-configured': return { color: 'outline', text: 'Not Configured' };
