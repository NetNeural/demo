...
export default function () {
  ...
  const r3 = http.get(`${BASE_URL}/rest/v1/alerts?select=id&status=eq.active`, {
...
export function handleSummary(data) {
  ...
  console.log(
    `Pool Error Rate:   ${(errRate * 100).toFixed(2)}% (threshold: 5%)`
  )
  console.log(
    `Result: ${errRate < 0.05 && p95 < 1000 ? '✅ PASS' : '❌ FAIL — log as follow-up issue'}`
  )
