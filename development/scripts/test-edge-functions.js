...
async function testEdgeFunctions() {
  ...
  const netNeural = orgsData.organizations?.find(
    (org) => org.name === 'NetNeural Demo'
  )
  if (netNeural) {
    console.log(
      `   NetNeural Demo: userCount = ${netNeural.userCount}, deviceCount = ${netNeural.deviceCount}`
    )
  }
  ...
  console.log('\n✅ Summary:')
  console.log(
    `   Organizations endpoint: ${netNeural?.userCount === 6 ? '✅ CORRECT' : '❌ WRONG'} (expected 6, got ${netNeural?.userCount})`
  )
  console.log(
    `   Dashboard-stats endpoint: ${statsData.totalUsers === 6 ? '✅ CORRECT' : '❌ WRONG'} (expected 6, got ${statsData.totalUsers})`
  )
}
