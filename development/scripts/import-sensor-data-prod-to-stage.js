...
const STAGE_SUPABASE_URL = process.env.STAGE_SUPABASE_URL || 'https://atgbmxicqikmapfqouco.supabase.co';
  ...
  try {
    ...
    for (const stagingDevice of stagingDevices) {
      ...
      const { data: prodDevices, error: prodError } = await prodClient
        .from('devices')
        .select('id, name, serial_number, external_device_id')
        .or(`serial_number.eq.${stagingDevice.serial_number || 'null'},external_device_id.eq.${stagingDevice.external_device_id || 'null'}`)
        .is('deleted_at', null)
        .limit(1);

      if (prodError) {
        console.error(`   ⚠️  Error fetching production device: ${prodError.message}`);
        continue;
      }
      ...
      console.log(`   ✅ Found matching production device (ID: ${prodDevice.id})`);

      // Step 3: Get telemetry data from production for this device
      console.log(`   📥 Fetching telemetry data from production (last ${DAYS_TO_IMPORT} days)...`);
      ...
      if (telemetryError) {
        console.error(`   ⚠️  Error fetching telemetry: ${telemetryError.message}`);
        continue;
      }
      ...
      const { data: existingTelemetry, error: existingError } = await stageClient
        .from('device_telemetry_history')
        .select('received_at')
      ...
      if (existingError) {
        console.error(`   ⚠️  Error checking existing telemetry: ${existingError.message}`);
        continue;
      }
      ...
      console.log(`   📤 Importing ${newTelemetry.length} new telemetry records...`);
