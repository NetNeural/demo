...
async function updateBucketLimit() {
  ...
  try {
    ...
    const { data, error } = await supabase.storage.updateBucket('organization-assets', {
      public: true,
    ...
    console.log('  - Rationale: Logos compressed to WebP at 400x400px typically <200KB');
    ...
    if (orgBucket) {
      ...
      console.log(`  - Allowed types: ${orgBucket.allowed_mime_types?.join(', ')}`);
    }
