...
async function checkWebhookConfig() {
  ...
  console.log(
    `   Secret: ${integration.webhook_secret ? '✅ SET (' + integration.webhook_secret.length + ' chars)' : '❌ NOT SET'}`
  )
  console.log('')
  ...
  if (!integration.webhook_secret) {
    ...
    console.log(
      '   This is OK for testing, but consider setting a secret for production'
    )
    console.log('')
  ...
  if (logError) {
    ...
    logs.forEach((log, index) => {
      ...
      console.log(
        `      Signature Verification: ${log.metadata?.signature_verification || 'unknown'}`
      )
