...
async function diagnoseUploadIssue() {
  ...
  if (!membership || membership.length === 0) {
    ...
    if (membership[0].role !== 'owner') {
      console.log(
        `⚠️  Role is "${membership[0].role}", updating to "owner"...\n`
      )
  ...
  const { data: policyTest, error: policyError } = await supabase.rpc(
    'exec_sql',
    {
      sql: `
  ...
  console.log(
    'Membership: ' + (membership && membership.length > 0 ? '✅ Yes' : '❌ No')
  )
  console.log(
    'Role: ' +
      (membership && membership[0]?.role === 'owner'
        ? '✅ Owner'
        : '❌ Not owner')
  )
