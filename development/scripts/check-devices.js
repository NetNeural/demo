...
async function checkDevices() {
  ...
  const { data: devices, error } = await supabase
    .from('devices')
    .select('id, name, serial_number, external_device_id, device_type, status, metadata, integration_id')
  ...
  devices.forEach((device, index) => {
    ...
    console.log(`   External Device ID: ${device.external_device_id || '(none)'}`);
    ...
    if (!device.serial_number) {
      ...
      else if (device.external_device_id && /^[A-Z]\d+$/.test(device.external_device_id)) {
        proposedSerial = device.external_device_id;
        console.log(`   💡 Using external_device_id as serial: ${proposedSerial}`);
      }
  ...
  if (updates.length > 0) {
    ...
    for (const update of updates) {
      ...
      if (updateError) {
        ...
        console.log(`✅ Updated device ${update.id} with serial: ${update.serial_number}`);
      }
    }
