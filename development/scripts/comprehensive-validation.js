...
async function testDashboard(page) {
  ...
  if (await alertsCard.isVisible()) {
    ...
    if (hasNoAlertsText) {
      logBug(
        'Dashboard',
        'Data',
        'Alerts showing "No alerts" - may need data seeding',
        'low'
      )
    }
  } else {
  ...
  if (viewAllCount >= 2) {
    ...
    logBug(
      'Dashboard',
      'UI',
      `Only ${viewAllCount} "View All" buttons found`,
      'low'
    )
  }
}
...
async function testPreferencesTab(page) {
  ...
  if (await preferencesTab.isVisible()) {
    ...
    await page.screenshot({
      path: 'validation-preferences.png',
      fullPage: true,
    })
    ...
    if (selects.length >= 3) {
      logPass(
        'Preferences Tab',
        `${selects.length} dropdowns found (Language, Timezone, Date Format)`
      )
    } else {
      logBug(
        'Preferences Tab',
        'UI',
        `Only ${selects.length} dropdowns found, expected at least 3`,
        'medium'
      )
    }
    ...
    if (await saveButton.isVisible()) {
      ...
      if (pageContent.includes('success') || pageContent.includes('saved')) {
        ...
        logBug(
          'Preferences Tab',
          'Functionality',
          'Save Preferences gave no feedback',
          'medium'
        )
      }
    } else {
      logBug(
        'Preferences Tab',
        'UI',
        'Save Preferences button not visible',
        'high'
      )
    }
  } else {
  ...
  if (await securityTab.isVisible()) {
    ...
    if ((await passwordSection.count()) > 0) {
      ...
      if (passwordFields.length >= 3) {
        logPass(
          'Security Tab',
          `${passwordFields.length} password fields found`
        )
      } else {
        logBug(
          'Security Tab',
          'UI',
          `Only ${passwordFields.length} password fields, expected 3`,
          'medium'
        )
      }
    } else {
    ...
    if ((await sessionsSection.count()) > 0) {
      ...
      logBug(
        'Security Tab',
        'UI',
        'Active Sessions section not found',
        'medium'
      )
    }
...
async function testOrganizations(page) {
  ...
  await page.waitForLoadState('networkidle')
  await page.screenshot({
    path: 'validation-organizations.png',
    fullPage: true,
  })
  ...
  if (orgCount > 0) {
      ...
      if (await tabElement.isVisible()) {
        ...
        if (tab.hasAddButton) {
          const addButton = page
            .locator(`button:has-text("${tab.buttonText}")`)
            .or(page.locator('button:has-text("Add")'))
          if ((await addButton.count()) > 0) {
            logPass(
              `Organization ${tab.name} Tab`,
              `"${tab.buttonText}" button visible`
            )
            ...
            if (hasDialog) {
              logPass(
                `Organization ${tab.name} Tab`,
                'Add button triggered modal/dialog'
              )
            }
          } else {
            logBug(
              `Organization ${tab.name} Tab`,
              'UI',
              `"${tab.buttonText}" button not found`,
              'medium'
            )
          }
        }
    ...
    logBug(
      'Organizations Page',
      'Data',
      'No organizations found - may need data seeding',
      'medium'
    )
  }
}
...
async function testDevices(page) {
  ...
  const deviceItems = page.locator(
    '[data-testid*="device"], .device-card, .device-item'
  )
  ...
  if (deviceCount > 0) {
    ...
    logBug(
      'Devices Page',
      'Data',
      'No devices found - may need data seeding',
      'low'
    )
  }
}
...
async function testAlerts(page) {
  ...
  const alertItems = page.locator(
    '[data-testid*="alert"], .alert-card, .alert-item'
  )
  ...
  if (alertCount > 0) {
    ...
    if ((await noAlerts.count()) > 0) {
      logBug(
        'Alerts Page',
        'Data',
        'No alerts in database - may need data seeding',
        'low'
      )
    }
  }
  ...
  const filterElements = page.locator(
    'select, input[type="search"], button:has-text("Filter")'
  )
  ...
  for (const pagePath of pages) {
    ...
    if (pageErrors.length > 0) {
      logBug(
        pagePath,
        'Console Errors',
        `${pageErrors.length} error(s) found`,
        'high'
      )
    ...
    if (await navLink.isVisible()) {
      ...
      if (url.includes(item.url)) {
        ...
        logBug(
          'Navigation',
          'Functionality',
          `"${item.text}" link went to wrong page: ${url}`,
          'high'
        )
      }
    } else {
...
async function generateReport() {
  ...
  if (bugs.length > 0) {
    ...
    if (critical.length > 0) {
      ...
      critical.forEach((b) =>
        console.log(`   - ${b.page} (${b.category}): ${b.issue}`)
      )
      console.log('')
    }
