...
async function getAllOrganizations() {
  ...
  const result = await makeRequest(
    'GET',
    '/rest/v1/organizations?select=id,name'
  )
...
async function getDeviceTypeCount(orgId) {
  const result = await makeRequest(
    'GET',
    `/rest/v1/device_types?organization_id=eq.${orgId}&select=id`
  )
...
async function seedDeviceTypesForOrg(orgId) {
  await makeRequest('POST', '/rest/v1/rpc/seed_organization_device_types', {
    org_id: orgId,
  })
}
  ...
  try {
    ...
    for (const org of organizations) {
      try {
        ...
        if (existingCount > 0) {
          console.log(
            `⏭️  Skipping "${org.name}" (already has ${existingCount} device types)`
          )
          skipped++
          continue
        }
    ...
    if (errors > 0) {
      console.error(
        '⚠️  Some organizations failed to backfill. Check errors above.'
      )
      process.exit(1)
    } else {
      console.log(
