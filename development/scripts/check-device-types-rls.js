...
async function checkRLS() {
  ...
  const serviceResponse = await fetch(
    `${STAGING_URL}/rest/v1/device_types?select=id,name,organization_id&limit=5`,
    {
      headers: {
  ...
  serviceData.forEach((dt) => {
    console.log(
      `   - ${dt.name} (org: ${dt.organization_id.substring(0, 8)}...)`
    )
  })
  ...
  const policiesResponse = await fetch(
    `${STAGING_URL}/rest/v1/rpc/pg_policies?select=*`,
    {
      headers: {
        apikey: STAGING_SERVICE_KEY,
        Authorization: `Bearer ${STAGING_SERVICE_KEY}`,
  ...
  if (STAGING_ANON_KEY) {
    ...
    const anonResponse = await fetch(
      `${STAGING_URL}/rest/v1/device_types?select=id,name&limit=5`,
      {
        headers: {
    ...
    console.log(
      `   Found ${anonData.length || 0} device types (unauthenticated)`
    )
  }
}
