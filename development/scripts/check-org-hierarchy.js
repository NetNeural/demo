...
async function checkHierarchy() {
  ...
  const response = await fetch(`${STAGING_URL}/rest/v1/organizations?select=id,name,slug,parent_organization_id&order=name.asc`, {
    headers: {
  ...
  rootOrgs.forEach(org => {
    ...
    // Find children
    const children = childOrgs.filter(c => c.parent_organization_id === org.id);
  ...
  childOrgs.forEach(org => {
    ...
    console.log(`  • ${org.name} (${org.slug})`);
    console.log(`    ID: ${org.id}`);
    console.log(`    Parent: ${parent ? parent.name : org.parent_organization_id}`);
    console.log('');
  });
  
  console.log('═'.repeat(80));
  console.log(`📊 Summary: ${rootOrgs.length} root orgs, ${childOrgs.length} child orgs`);
}
