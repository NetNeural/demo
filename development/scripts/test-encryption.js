...
async function testEncryption() {
  ...
  try {
    ...
    const encryptResult = await makeRequest('POST', '/rest/v1/rpc/encrypt_api_key', {
    ...
    const decryptResult = await makeRequest('POST', '/rest/v1/rpc/decrypt_api_key', {
    ...
    if (decryptedKey === testKey) {
      ...
      throw new Error(`Round-trip verification FAILED: "${testKey}" !== "${decryptedKey}"`);
    }
    
    // Test 4: Test deterministic encryption
    console.log('\n4️⃣  Testing deterministic encryption...');
    const encryptResult2 = await makeRequest('POST', '/rest/v1/rpc/encrypt_api_key', {
      plaintext_key: testKey,
      p_key_id: 'default'
    });
    ...
    if (encryptedKey === encryptedKey2) {
      console.log('✅ Deterministic encryption PASSED (same input = same output)');
    } else {
      console.log('⚠️  Deterministic encryption note: Different outputs (expected for AEAD)');
    }
    ...
    const fallbackResult = await makeRequest('POST', '/rest/v1/rpc/decrypt_api_key', {
