...
async function testMqttBroker() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        '   (This may be expected if mqtt-hybrid needs npm:mqtt support)'
      )
    }
    ...
    try {
      const publishResult = await makeRequest(
        'POST',
        '/functions/v1/mqtt-hybrid/publish',
        {
          integration_id: integrationId,
          organization_id: ORG_ID,
          messages: [
      ...
      console.log(
        `✅ Publish result: ${publishResult.body.data.results.published} messages published`
      )
    } catch (error) {
    ...
    try {
      const subscribeResult = await makeRequest(
        'POST',
        '/functions/v1/mqtt-hybrid/subscribe',
        {
          integration_id: integrationId,
          organization_id: ORG_ID,
