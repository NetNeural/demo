...
async function checkDuplicates() {
  ...
  const { data: bySerial, error: serialError } = await supabase
    .from('devices')
    .select('id, name, serial_number, external_device_id, status, created_at, updated_at')
  ...
  if (serialError) {
    ...
    console.log('Devices with serial_number containing M260600010:', bySerial?.length || 0);
  ...
  if (externalError) {
    ...
    console.log('\n\nDevices with external_device_id containing M260600010:', byExternal?.length || 0);
