...
const SUPABASE_URL = Deno.env.get('SUPABASE_URL') || 'https://bldojxpockljyivldxwf.supabase.co';
...
async function testProductionPage(name: string, path: string): Promise<TestResult> {
  try {
    ...
    if (html.includes('infinite recursion') || html.includes('recursion detected')) {
      return {
        name,
        passed: false,
        status: response.status,
    ...
    if (html.includes('policy for relation') && html.includes('organization_members')) {
      return {
        name,
        passed: false,
        status: response.status,
...
async function checkMigrationStatus(): Promise<TestResult> {
  try {
    ...
    const testQuery = await fetch(`${SUPABASE_URL}/rest/v1/organization_members?select=id&limit=1`, {
      headers: {
        'apikey': SUPABASE_ANON_KEY || '',
      },
    });
...
async function runTests() {
  ...
  console.log(`  Anon Key: ${SUPABASE_ANON_KEY ? '***' + SUPABASE_ANON_KEY.slice(-8) : 'Not set'}\n`);
  ...
  results.push(await testProductionPage('Organizations', '/dashboard/organizations'));
  ...
  results.push(await testEdgeFunction('Devices (POST)', 'devices', 'POST', { name: 'test' }));
  results.push(await testEdgeFunction('Members (GET)', 'members'));
  results.push(await testEdgeFunction('Members (POST)', 'members', 'POST', { email: 'test@test.com' }));
  results.push(await testEdgeFunction('Integrations (GET)', 'integrations'));
  results.push(await testEdgeFunction('Integrations (POST)', 'integrations', 'POST', { name: 'test' }));
  results.push(await testEdgeFunction('Locations (GET)', 'locations'));
  results.push(await testEdgeFunction('Locations (PATCH)', 'locations', 'PATCH', { name: 'test' }));
  results.push(await testEdgeFunction('Locations (DELETE)', 'locations', 'DELETE'));
  results.push(await testEdgeFunction('Organizations (GET)', 'organizations'));
  results.push(await testEdgeFunction('Organizations (PATCH)', 'organizations', 'PATCH', { name: 'test' }));
  results.push(await testEdgeFunction('Dashboard Stats', 'dashboard-stats'));
  results.push(await testEdgeFunction('Create User', 'create-user', 'POST', { email: 'test@test.com' }));
  ...
  if (failed === 0) {
    ...
    log(colors.red, `✗ SOME TESTS FAILED (${passed}/${total} passed, ${failed} failed)\n`);
  }
  ...
  const criticalFailed = results.some(r => 
    !r.passed && (r.error?.includes('recursion') || r.error?.includes('not deployed'))
  );
