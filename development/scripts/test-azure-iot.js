...
async function testAzureIoTHub() {
  ...
  try {
    ...
    try {
      ...
      } = require('../src/lib/integrations/azure-iot-integration-provider.ts')
      ...
      console.log(
        '   (This may be expected if running in Node.js without TypeScript setup)'
      )
    }
    ...
    try {
      ...
      console.log(
        `✅ Found ${deviceList.total} devices (showing ${deviceList.devices.length}):`
      )
      deviceList.devices.forEach((device, index) => {
        console.log(
          `   ${index + 1}. ${device.name} (${device.id}) - Status: ${device.status}`
        )
      })
      ...
      if (deviceList.devices.length > 0) {
        ...
        console.log(
          `   Firmware: ${deviceStatus.firmware?.version || 'unknown'}`
        )
    ...
    try {
      ...
      if (telemetry.length === 0) {
        console.log(
          `✅ Telemetry query returned empty (expected - Azure IoT Hub doesn't store telemetry)`
        )
        console.log(
          '   Note: Configure Azure IoT Central, Time Series Insights, or Data Explorer for telemetry storage'
        )
      } else {
        console.log(
          `⚠️  Unexpected: Telemetry query returned ${telemetry.length} records`
        )
      }
    } catch (error) {
    ...
    activities.forEach((activity, index) => {
      console.log(
        `   ${index + 1}. ${activity.activity_type} - ${activity.status} - ${activity.message}`
      )
    })
    ...
    await makeRequest(
      'DELETE',
      `/rest/v1/device_integrations?id=eq.${integrationId}`
    )
