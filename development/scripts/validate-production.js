    ...
    const req = https.get(url, options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (status >= 200 && status < 300) {
          ...
          console.log(`   ⚠️  EXPECTED ERROR: ${status} (service is responding)`);
...
async function validateProduction() {
  ...
  if (results.warnings.length > 0) {
    ...
    results.warnings.forEach(r => console.log(`   - ${r.name}: ${r.status || r.error}`));
  }

  if (results.failed.length > 0) {
    console.log('\n❌ FAILED TESTS:');
    results.failed.forEach(r => console.log(`   - ${r.name}: ${r.error || r.status}`));
  }
  ...
  if (results.failed.length === 0) {
    ...
    console.log(`   ${totalWorking}/${totalTests} endpoints responding correctly`);
    console.log('═══════════════════════════════════════════════════════');
    process.exit(0);
  } else {
