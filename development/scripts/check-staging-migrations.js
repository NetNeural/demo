...
const STAGING_URL = process.env.STAGING_URL || 'https://atgbmxicqikmapfqouco.supabase.co';
...
if (!STAGING_SERVICE_ROLE_KEY) {
  ...
  console.error('');
  console.error('Usage:');
  console.error('  STAGING_SERVICE_ROLE_KEY=<your-key> node scripts/check-staging-migrations.js');
  console.error('');
  console.error('Or get it from GitHub secrets:');
  console.error('  gh secret list');
  console.error('  export STAGING_SERVICE_ROLE_KEY=$(gh secret get STAGING_SERVICE_ROLE_KEY)');
...
async function checkStagingMigrations() {
  try {
    ...
    if (!response.ok) {
      console.error('❌ Failed to query staging migrations table:', response.status, response.error);
      console.log('\n⚠️  Cannot verify migrations. Table may not exist or is not accessible.');
      console.log('💡 Possible table names: supabase_migrations, _supabase_migrations, schema_migrations');
...
function compareMigrations(localFiles, appliedVersions) {
  ...
  if (missing.length > 0) {
    ...
    console.log('   3. Critical for AI Analytics: 20250109000003_telemetry_all_integrations.sql');
  }
}
