  ...
  try {
    ...
    if (rememberMe === 0) {
      bugs.push({
        ...
        page: 'Login',
        message: 'Remember Me checkbox not found',
      })
    ...
    if (!currentUrl.includes('/dashboard')) {
      bugs.push({
        ...
        page: 'Login',
        message: `Did not redirect to dashboard. URL: ${currentUrl}`,
      })
      console.log(
        `❌ Bug: Login did not redirect to dashboard (URL: ${currentUrl})`
      )
    } else {
    ...
    const alertsCard = await page
      .locator('text=Alert')
      .or(page.locator('h2:has-text("Alerts")'))
      .or(page.locator('h3:has-text("Alerts")'))
      .count()
    if (alertsCard === 0) {
      bugs.push({
        type: 'Missing Component',
        page: 'Dashboard',
        message: 'Bug #7: Alerts card not visible',
      })
    ...
    const locationsCard = await page
      .locator('text=Location')
      .or(page.locator('h2:has-text("Locations")'))
      .or(page.locator('h3:has-text("Locations")'))
      .count()
    if (locationsCard === 0) {
      bugs.push({
        type: 'Missing Component',
        page: 'Dashboard',
        message: 'Bug #12: Locations card not visible',
      })
    ...
    if (spinners > 0) {
      warnings.push({
        page: 'Dashboard',
        message: `${spinners} loading spinners still visible`,
      })
    ...
    if (errorMessages > 0) {
      bugs.push({
        type: 'Error Message',
        page: 'Dashboard',
        message: `${errorMessages} error messages displayed`,
      })
    ...
    if (await profileTab.isVisible()) {
      ...
      if (saveButton === 0) {
        bugs.push({
          ...
          message: 'Bug #11: Save button not found',
        })
      ...
      bugs.push({
        type: 'Missing Tab',
        page: 'Settings',
        message: 'Profile tab not visible',
      })
      console.log('❌ Profile tab not found')
    }

    await page.screenshot({
      path: 'bug-report-settings-profile.png',
      fullPage: true,
    })
    ...
    if (await preferencesTab.isVisible()) {
      ...
      if (savePrefButton > 0) {
        ...
        bugs.push({
          type: 'Missing Element',
          page: 'Settings/Preferences',
          message: 'Bug #16: Save button not found',
        })
      ...
      console.log(
        `✅ Found ${selects} dropdown(s) (Language, Timezone, Date Format)`
      )
    } else {
      bugs.push({
        type: 'Missing Tab',
        page: 'Settings',
        message: 'Preferences tab not visible',
      })
      console.log('❌ Preferences tab not found')
    }

    await page.screenshot({
      path: 'bug-report-settings-preferences.png',
      fullPage: true,
    })
    ...
    if (await securityTab.isVisible()) {
      ...
      const passwordSection = await page
        .locator('text=/change password|current password/i')
        .count()
      if (passwordSection > 0) {
        ...
        bugs.push({
          ...
          message: 'Bug #18: Change Password section not found',
        })
      ...
      if (twoFASection > 0) {
        ...
        bugs.push({
          type: 'Missing Section',
          page: 'Settings/Security',
          message: 'Bug #19: 2FA section not found',
        })
        console.log('❌ Bug #19: 2FA section missing')
      }

      // Check active sessions (Bug #20)
      const sessionsSection = await page
        .locator('text=/active session|session/i')
        .count()
      if (sessionsSection > 0) {
        ...
        bugs.push({
          type: 'Missing Section',
          page: 'Settings/Security',
          message: 'Bug #20: Active Sessions section not found',
        })
      ...
      if (apiKeysSection > 0) {
        ...
        bugs.push({
          type: 'Missing Section',
          page: 'Settings/Security',
          message: 'Bug #21: API Keys section not found',
        })
      ...
      bugs.push({
        type: 'Missing Tab',
        page: 'Settings',
        message: 'Security tab not visible',
      })
      console.log('❌ Security tab not found')
    }

    await page.screenshot({
      path: 'bug-report-settings-security.png',
      fullPage: true,
    })
    ...
    const firstOrg = await page
      .locator('a, button')
      .filter({ hasText: /netneural|organization|view/i })
      .first()
    if (await firstOrg.isVisible().catch(() => false)) {
      ...
      const tabs = {
        ...
        '#4 Integrations':
          (await page.locator('text=Integrations').count()) > 0,
      ...
      if (addDeviceBtn > 0) {
        ...
        bugs.push({
          ...
          message: 'Bug #1: Add Device button not found',
        })
    ...
    await page.screenshot({
      path: 'bug-report-organizations.png',
      fullPage: true,
    })
    ...
    console.log('\n📄 PAGE 7: DEVICES')
    console.log('-'.repeat(60))
    await page.goto('http://localhost:3004/dashboard/devices/', {
      waitUntil: 'networkidle',
    })
    await page.waitForTimeout(2000)
    ...
    console.log('\n📄 PAGE 8: ALERTS')
    console.log('-'.repeat(60))
    await page.goto('http://localhost:3004/dashboard/alerts/', {
      waitUntil: 'networkidle',
    })
    await page.waitForTimeout(2000)
