...
async function checkIntegrations() {
  ...
  if (integrations.length === 0) {
    ...
    integrations.forEach((int, i) => {
      ...
      console.log(`     Device: ${int.devices?.name || 'N/A'} (${int.devices?.device_id || 'N/A'})`)
      console.log(`     Organization: ${int.organizations?.name || 'N/A'}`)
      console.log(`     Enabled: ${int.enabled}`)
      console.log(`     Config: ${JSON.stringify(int.config).substring(0, 100)}...`)
      console.log()
    })
  }
  
  console.log('\n💡 I need production database credentials to copy integrations.')
  console.log('   What are the Supabase URL and service role key for production?')
}
