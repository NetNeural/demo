...
if (localCommit.success && remoteCommit.success) {
  ...
  if (local === remote) {
    ...
    const unpushed = exec('git log origin/main..HEAD --oneline', { silent: true });
...
log('Would you like to manually trigger workflows? (requires user confirmation)', 'yellow');
...
try {
  ...
  if (pkgJson.devDependencies?.['@testing-library/dom']) {
    log(`✅ @testing-library/dom found: ${pkgJson.devDependencies['@testing-library/dom']}`, 'green');
  } else {
...
log('2. View workflows at: https://github.com/NetNeural/MonoRepo-Staging/actions', 'cyan');
log('3. Manually trigger test workflow: gh workflow run test.yml', 'cyan');
log('4. Manually trigger deploy: gh workflow run deploy-staging.yml -f force_deploy=true', 'cyan');
log('5. Check staging site: https://demo-stage.netneural.ai/dashboard/devices/', 'cyan');
