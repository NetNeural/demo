...
class MqttSubscriberService {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (integrations.length === 0) {
        logger.warn(
          '⚠️  No active MQTT integrations found. Service will check again in 60 seconds.'
        )
  ...
  private async connectToIntegration(
    ...
    try {
      ...
      const clientId =
        settings.clientId || `netneural-subscriber-${id}-${Date.now()}`
        ...
        if (topics.length > 0) {
          client.subscribe(topics, (err) => {
            if (err) {
              logger.error(
                { err, topics },
                `Failed to subscribe to topics for ${name}`
              )
            } else {
              logger.info(
                { topics },
                `📡 Subscribed to ${topics.length} topic(s) for ${name}`
              )
            }
          })
        } else {
  ...
  private parseTopics(topicsConfig: any): string[] {
    ...
    return ['devices/+/telemetry', 'devices/+/status', 'netneural/+/data']
  }
