    ...
    for (const user of users) {
      ...
      if (userMembs.length === 0) {
        if (user.organization_id) {
          issues.push({ user, issue: `Has org_id (${userOrgName}) but no memberships` })
        }
      } else {
        const match = userMembs.find(m => m.organization_id === user.organization_id)
        if (!match) {
          const membOrgNames = userMembs.map(m => orgMap.get(m.organization_id)).join(', ')
          issues.push({ 
            user, 
