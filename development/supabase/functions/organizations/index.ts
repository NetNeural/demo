    ...
    try {
        ...
        try {
          ...
          if (authHeader) {
            try {
              ...
              if (parts.length >= 2) {
                const payload = JSON.parse(globalThis.atob(parts[1]))
                console.log('Using token payload:', {
                  sub: payload.sub,
                  role: payload.role,
                })
                ...
                if (!payload.sub) {
                  throw new Error(
                    'JWT token has no sub claim — cannot identify user'
                  )
                }
                ...
                if (profile) {
                  ...
                  console.log(
                    '✅ Constructed userContext from token:',
                    userContext
                  )
                } else {
                  console.error('No profile found for user:', payload.sub)
                  throw new Error(
                    `No user profile found for ID: ${payload.sub}`
                  )
                }
              }
            } catch (tokenError) {
              ...
              throw new DatabaseError(
                `Token parsing failed: ${tokenError.message}`,
                401
              )
            }
          }
      ...
      if (req.method === 'GET') {
        ...
        if (parentOrgFilter) {
          if (!userContext.isSuperAdmin) {
            ...
            const isUserDefaultOrg =
              userContext.organizationId === parentOrgFilter
            const isDefaultOrgAdmin =
              isUserDefaultOrg && ['owner', 'admin'].includes(userContext.role)
            ...
            if (!isDefaultOrgAdmin && !isOrgCreator && !isMemberAdmin) {
              throw new DatabaseError(
                'You must be an owner or admin of the parent organization to view child orgs',
                403
              )
            }
          }
          ...
          const { data: descendants, error: descendantsError } =
            await supabaseAdmin.rpc('get_all_descendant_organizations', {
              parent_org_id: parentOrgFilter,
            })

          if (descendantsError) {
            console.error(
              'Error fetching descendant organizations:',
              descendantsError
            )
            throw new DatabaseError(
              `Failed to fetch descendant organizations: ${descendantsError.message}`
            )
          }
        ...
        if (!userContext.isSuperAdmin) {
          ...
          if (
            ...
            !memberOrgIds.includes(userContext.organizationId)
          ) {
        ...
        const enrichedOrgs = await Promise.all(
          (organizations || []).map(async (org: any) => {
            ...
            return {
              ...
              deviceCount: deviceCount || 0,
              alertCount: alertCount || 0,
              memberRole: userContext.isSuperAdmin
                ? 'owner'
                : membershipMap[org.id] || 'viewer',
              createdAt: org.created_at,
              updatedAt: org.updated_at,
            }
          })
      ...
      if (req.method === 'POST') {
        ...
        if (parentOrganizationId) {
          ...
          if (!resellerTiers.includes(parentOrg.subscription_tier || '')) {
            throw new DatabaseError(
              'Parent organization does not have reseller privileges',
              403
            )
          }
          ...
          if (!userContext.isSuperAdmin) {
            ...
            if (!membership || !['owner', 'admin'].includes(membership.role)) {
              throw new DatabaseError(
                'You must be an owner or admin of the parent organization',
                403
              )
            }
          }
          ...
          if (agreement) {
            ...
            if (
              ...
              (childCount || 0) >= agreement.max_child_organizations
            ) {
              throw new DatabaseError(
          ...
          console.log(
            `Creating child org under parent: ${parentOrg.name} (${parentOrganizationId})`
          )
        }
        ...
        if (checkError) {
          ...
          throw new DatabaseError(
            `Failed to check existing slug: ${checkError.message}`
          )
        }
      ...
      if (req.method === 'DELETE') {
        ...
        if (deleteError) {
          ...
          throw new DatabaseError(
            `Failed to delete organization: ${deleteError.message}`
          )
        }
