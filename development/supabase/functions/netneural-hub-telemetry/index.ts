...
import { createEdgeFunction, createSuccessResponse, createErrorResponse } from '../_shared/create-edge-function.ts'
...
function detectProtocol(req: Request): string {
  ...
  if (contentType?.includes('application/cbor') || userAgent?.includes('coap')) {
    return 'coap'
  }
  
  // MQTT bridge requests
  if (contentType?.includes('application/json') && userAgent?.includes('mqtt')) {
    return 'mqtt'
  }
...
function extractDeviceId(req: Request, body: any, _protocol: string): string | null {
...
function normalizeTelemetry(data: any, protocol: string, _deviceType?: string): Record<string, unknown> {
...
// Main Edge Function
export default createEdgeFunction(async ({ request, supabase, url: _url }) => {
  // Only allow POST requests for telemetry
  if (request.method !== 'POST') {
    return createErrorResponse('Only POST method allowed for telemetry ingestion', 405)
  }
  ...
  try {
    ...
    if (deviceError || !device) {
      ...
      return createErrorResponse('Device not registered in NetNeural Hub', 404)
    }
      
    // Normalize telemetry data
    const telemetryData = normalizeTelemetry(body, protocol, device.device_type)
    ...
    if (telemetryError) {
      console.error(`[NetNeuralHub] Failed to store telemetry for ${deviceId}:`, telemetryError)
  ...
  try {
    ...
    for (const rule of alertRules) {
      ...
      if (triggered) {
        ...
        console.log(`[NetNeuralHub] 🚨 Alert triggered for ${device.name}: ${field} ${operator} ${threshold}`)
      }
    }
  } catch (error) {
