...
serve(async (req) => {
  try {
    ...
    const { reportType, dateRange, totalRecords, data, organizationId } = reportData
...
{"keyFindings": [...], "redFlags": [...], "recommendations": [...], "trendAnalysis": "..."}`
    ...
    summary = {
      keyFindings: (summary.keyFindings || []).slice(0, 5).map(s => String(s).substring(0, 80)),
      redFlags: (summary.redFlags || []).slice(0, 3).map(s => String(s).substring(0, 100)),
      recommendations: (summary.recommendations || []).slice(0, 5).map(s => String(s).substring(0, 100)),
...
function prepareReportSummary(reportData: ReportData): string {
  ...
  switch (reportType) {
    case 'alert_history': {
      const alerts = data as Array<{ severity: string; is_resolved: boolean; alert_type: string }>
...
- Top Categories: ${Object.entries(byCategory).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([k, v]) => `${k}(${v})`).join(', ')}`
    }
