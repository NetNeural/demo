...
serve(async (req) => {
  try {
    ...
    if (!openaiApiKey) {
      console.warn('⚠️ OPENAI_API_KEY not configured - returning fallback insights')
      return new Response(
        JSON.stringify({
    ...
    const { deviceId, deviceName, installedAt, telemetryReadings, temperatureUnit, organizationId } = requestData
    ...
    const dataSummary = prepareTelemetrySummary(telemetryReadings, temperatureUnit)
...
function prepareTelemetrySummary(readings: TelemetryReading[], unit: string): string {
  ...
  for (const reading of readings) {
    const sensorKey = reading.telemetry.sensor || `type_${reading.telemetry.type}` || 'unknown'
  ...
  const timespan = oldestReading?.received_at 
    ? Math.round((now - new Date(oldestReading.received_at).getTime()) / (1000 * 60))
    : 0
  ...
  for (const [sensor, values] of Object.entries(sensorGroups)) {
    ...
    const recentAvg = values.slice(0, halfPoint).reduce((a, b) => a + b, 0) / halfPoint
    const olderAvg = values.slice(halfPoint).reduce((a, b) => a + b, 0) / (values.length - halfPoint)
    const trendPct = ((recentAvg - olderAvg) / olderAvg * 100).toFixed(1)
    const trend = Math.abs(parseFloat(trendPct)) > 5 ? (trendPct > 0 ? '↑' : '↓') : '→'
    
    const unitStr = sensor.includes('temp') ? `°${unit === 'fahrenheit' ? 'F' : 'C'}` :
                    sensor.includes('humidity') ? '%' :
                    sensor.includes('pressure') ? ' hPa' : ''
    
    summary.push(`${sensor}: Current ${current.toFixed(1)}${unitStr} | Avg ${avg.toFixed(1)} | Range ${min.toFixed(1)}-${max.toFixed(1)} | Trend ${trend}${trendPct}%`)
  }
