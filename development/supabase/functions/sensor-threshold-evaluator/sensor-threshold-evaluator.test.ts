    ...
    from: (table: string) => ({
            ...
            order: (column: string, options: any) => ({
              limit: (n: number) => ({
                then: (callback: Function) =>
                  callback({ data: mockData.readings, error: null }),
              }),
            }),
          }),
          then: (callback: Function) =>
            callback({ data: mockData.thresholds, error: null }),
        }),
        then: (callback: Function) =>
          callback({ data: mockData.thresholds, error: null }),
      }),
      insert: (data: any) => ({
        select: () => ({
          single: () => ({
            then: (callback: Function) =>
              callback({ data: mockData.alert, error: null }),
          }),
        }),
      }),
      update: (data: any) => ({
...
Deno.test(
  'Temperature Unit Conversion - Negative Temperature (-40°C = -40°F)',
  () => {
...
Deno.test(
  'Fahrenheit Threshold with Celsius Reading - Conversion Required',
  () => {
    ...
    if (
      ...
      fahrenheitValue >= threshold.critical_max
    ) {
      breachType = 'critical_max'
    } else if (
      threshold.max_value != null &&
      fahrenheitValue > threshold.max_value
    ) {
      breachType = 'max'
    }
