...
export class CoapProtocolHandler extends ProtocolHandler {
  ...
  async test(): Promise<TestResult> {
    try {
      ...
      return {
        success: response.ok,
        message: response.ok
          ? 'CoAP endpoint reachable'
          : `CoAP endpoint error: ${response.status}`,
        details,
      }
    } catch (error) {
      const errorMessage =
  ...
  async sendTelemetry(
    ...
    try {
      ...
      const contentType = this.config.options?.use_cbor
        ? 'application/cbor'
        : 'application/json'
      ...
      const response = await fetch(
        `${this.config.endpoint}/v1/devices/${deviceId}/telemetry`,
        {
          method: 'POST',
          headers: {
      ...
      console.error(
        `[CoapHandler] Telemetry send failed for ${deviceId}:`,
        error
      )
      return false
    }
  ...
  async sendCommand(
    ...
    try {
      const response = await fetch(
        `${this.config.endpoint}/v1/devices/${deviceId}/commands`,
        {
          method: 'POST',
          headers: {
  ...
  async subscribe(
    ...
    try {
      ...
      ws.onerror = (error) => {
        console.error(
          `[CoapHandler] Observe connection error for ${deviceId}:`,
          error
        )
      }
      ...
      console.error(
        `[CoapHandler] Failed to establish observe for ${deviceId}:`,
        error
      )
    }
  }
...
export class MqttProtocolHandler extends ProtocolHandler {
    ...
    try {
      ...
      return {
        ...
        message: response.ok
          ...
          : `MQTT broker error: ${response.status}`,
        details,
      }
    } catch (error) {
      const errorMessage =
    ...
    try {
      ...
      const response = await fetch(`${this.config.endpoint}/publish`, {
        ...
        headers: {
          'Content-Type': 'application/json',
          'X-Protocol': 'mqtt',
          Authorization: this.config.auth?.credentials?.token
            ? `Bearer ${this.config.auth.credentials.token}`
            : '',
        },
        body: JSON.stringify({
      ...
      console.error(
        `[MqttHandler] Telemetry send failed for ${deviceId}:`,
        error
      )
      return false
    }
  ...
  async subscribe(
    ...
    try {
      ...
      ws.onerror = (error) => {
        console.error(
          `[MqttHandler] WebSocket MQTT connection error for ${deviceId}:`,
          error
        )
      }
      ...
      console.error(
        `[MqttHandler] Failed to establish MQTT subscription for ${deviceId}:`,
        error
      )
    }
  }
...
export class HttpsProtocolHandler extends ProtocolHandler {
    ...
    try {
      ...
      return {
        ...
        message: response.ok
          ...
          : `HTTPS endpoint error: ${response.status}`,
        details,
      }
    } catch (error) {
      const errorMessage =
  ...
  async sendTelemetry(
    ...
    try {
      ...
      const response = await fetch(
        `${this.config.endpoint}/v1/devices/${deviceId}/telemetry`,
        {
          ...
          body: JSON.stringify({
            ...
            delivery_receipt:
              this.config.options?.require_delivery_receipt || false,
          }),
        }
      )
      ...
      console.error(
        `[HttpsHandler] Telemetry send failed for ${deviceId}:`,
        error
      )
      return false
    }
  ...
  async sendCommand(
    ...
    try {
      ...
      console.error(
        `[HttpsHandler] Command send failed for ${deviceId}:`,
        error
      )
      return false
    }
  ...
  private subscribeSSE(
    ...
    try {
      ...
      eventSource.onerror = (error) => {
        console.error(
          `[HttpsHandler] SSE connection error for ${deviceId}:`,
          error
        )
      ...
      console.error(
        `[HttpsHandler] Failed to establish SSE for ${deviceId}:`,
        error
      )
      this.subscribePolling(deviceId, callback)
    }
  ...
  private subscribePolling(
    ...
    const pollData = async () => {
      try {
        const customHeaders = this.buildCustomHeaders()
        const response = await fetch(
          `${this.config.endpoint}/v1/devices/${deviceId}/status`,
          {
            headers: {
              Authorization: this.config.auth?.credentials?.token
                ? `Bearer ${this.config.auth.credentials.token}`
  ...
  private buildCustomHeaders(): Record<string, string> {
    ...
    if (
      this.config.options?.user_agent &&
      typeof this.config.options.user_agent === 'string'
    ) {
  ...
  normalizeDevice(rawDevice: Record<string, unknown>): Device {
    return {
      ...
      metadata: {
        ...
        firmware_version: (rawDevice.metadata as any)?.update?.[
  ...
  normalizeDevice(rawDevice: Record<string, unknown>): Device {
    return {
      ...
      metadata: {
        device_type: this.deviceType,
        firmware_version: (rawDevice.metadata as any)?.update?.[
          'test-bleconnectivity-nrf52840dk'
        ]?.version,
        ble_info: {
...
export class NetNeuralHubClient extends BaseIntegrationClient {
  ...
  protected validateConfig(): void {
    ...
    const enabledProtocols = Object.entries(settings.protocols).filter(
  ...
  private initializeProtocolHandlers(): void {
    if (this.hubConfig.protocols.coap?.enabled) {
      this.protocolHandlers.set(
        'coap',
        new CoapProtocolHandler(this.hubConfig.protocols.coap)
      )
    }
    ...
    if (this.hubConfig.protocols.mqtt?.enabled) {
      this.protocolHandlers.set(
        'mqtt',
        new MqttProtocolHandler(this.hubConfig.protocols.mqtt)
      )
    }
    ...
    if (this.hubConfig.protocols.https?.enabled) {
      this.protocolHandlers.set(
        'https',
        new HttpsProtocolHandler(this.hubConfig.protocols.https)
      )
    }
  }
      ...
      try {
        ...
        for (const device of devices) {
          try {
            ...
            const normalizedDevice = await this.normalizeDevice(
              device as any,
              protocol
            )

            // Insert/update device in database
            const { error } = await this.config.supabase.from('devices').upsert(
              {
            ...
            if (error) {
              errors.push(
                `Failed to save device ${device.name}: ${error.message}`
              )
              totalFailed++
            } else {
              totalSucceeded++
            ...
            const errorMsg =
              deviceError instanceof Error
                ...
                : 'Unknown error'
            errors.push(
              `Device processing failed for ${device.name}: ${errorMsg}`
            )
            totalFailed++
          }
        }
  ...
  public async export(devices: Device[]): Promise<SyncResult> {
    ...
    for (const device of devices) {
      ...
      try {
        ...
        const success = await handler.sendTelemetry(
          device.hardware_id || device.id,
          {
  ...
  private async normalizeDevice(
    ...
    return {
      ...
      hardware_id:
        (rawDevice.hardware_id as string) || (rawDevice.id as string),
      status: (rawDevice.status as Device['status']) || 'unknown',
  ...
  private detectDeviceType(rawDevice: Record<string, unknown>): string {
    ...
    if (
      hardwareId.includes('c252') ||
      hardwareId.includes('c253') ||
      name.toLowerCase().includes('c252') ||
      name.toLowerCase().includes('c253')
    ) {
      return 'nrf9151_cellular'
    }

    if (
      hardwareId.includes('nrf52840') ||
      name.toLowerCase().includes('nrf52840') ||
      metadata.firmware?.includes?.('nrf52840')
    ) {
      return 'nrf52840_ble'
    }

    if (
      hardwareId.includes('vmark') ||
      name.toLowerCase().includes('vmark') ||
      metadata.protocol === 'vmark'
    ) {
      return 'vmark_sensor'
    }
  ...
  private selectProtocolForDevice(device: Device): string {
    ...
    const routing =
      this.hubConfig.device_routing[deviceType] ||
      this.hubConfig.device_routing.default
   ...
   */
  public async addDeviceCapabilities(
    deviceType: string,
    capabilities: Array<{
      capability: string
      metadata: Record<string, unknown>
    ...
    if (error) {
      throw new IntegrationError(
        `Failed to add capabilities for ${deviceType}: ${error.message}`,
        'DB_ERROR'
      )
    }
