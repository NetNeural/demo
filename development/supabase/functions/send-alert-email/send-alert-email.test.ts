...
Deno.test('Email Deduplication - Removes Duplicate Emails', () => {
  const allEmails = ['user1@example.com', 'user2@example.com', 'user1@example.com', 'user3@example.com']
  const uniqueEmails = [...new Set(allEmails)]
  
  assertEquals(uniqueEmails.length, 3)
  assertEquals(uniqueEmails, ['user1@example.com', 'user2@example.com', 'user3@example.com'])
})
...
Deno.test('Alert Severity Color Mapping - Critical', () => {
  ...
  const headerColor = alert.severity === 'critical' ? '#dc2626' : alert.severity === 'high' ? '#ea580c' : '#0ea5e9'
...
Deno.test('HTML Email Structure - Escapes Newlines in Message', () => {
  ...
  assertEquals(escapedMessage, 'Temperature exceeded<br>Device: Sensor A1<br>Location: Warehouse')
...
Deno.test('Success Count - All Emails Sent Successfully', () => {
  const uniqueEmails = ['user1@example.com', 'user2@example.com', 'user3@example.com']
  const results = []
  let successCount = 0
