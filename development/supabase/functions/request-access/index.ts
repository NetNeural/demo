...
import { createEdgeFunction, createSuccessResponse, createErrorResponse, DatabaseError, getRequestBody } from '../_shared/request-handler.ts'
...
export default createEdgeFunction(async ({ req, userContext, method, url }) => {
  if (!userContext) {
  ...
  if (method === 'POST') {
    ...
    if (!interval) {
      throw new DatabaseError(`Invalid duration. Valid options: ${Object.keys(DURATION_MAP).join(', ')}`, 400)
    }
    ...
    if (!userContext.isSuperAdmin) {
      ...
      if (!membership || !['owner', 'admin'].includes(membership.role)) {
        throw new DatabaseError('Only admins or owners can request cross-org access', 403)
      }
    }
    ...
    if (target_org_id === userContext.organizationId) {
      throw new DatabaseError('Cannot request access to your own organization', 400)
    }
    ...
    if (existingRequest) {
      throw new DatabaseError('You already have a pending request for this organization', 409)
    }
    ...
    if (activeAccess) {
      throw new DatabaseError('You already have active access to this organization', 409)
    }
    ...
    if (targetOwners && targetOwners.length > 0) {
      try {
        ...
        console.log(`[request-access] Notification: ${requesterName} requests ${interval} access to ${targetOrg.name}`, {
          ownerUserIds,
  ...
  if (method === 'GET') {
    ...
    if (view === 'sent') {
      ...
      const resolvedOrgId = orgId || await resolveOrganizationId(userContext, orgId)
      if (resolvedOrgId) {
  ...
  if (method === 'PATCH') {
    ...
    const { data: request, error: fetchError } = await supabase
      .from('access_requests')
      .select('*, target_org:organizations!access_requests_target_org_id_fkey(id, name)')
      .eq('id', request_id)
      .single()
    ...
    if (request.status !== 'pending') {
      throw new DatabaseError(`Request has already been ${request.status}`, 409)
    }
    ...
    if (!userContext.isSuperAdmin) {
      ...
      if (!membership || !['owner', 'admin'].includes(membership.role)) {
        throw new DatabaseError('Only target org owners/admins can approve or deny requests', 403)
      }
    }
    ...
    if (action === 'approve') {
      ...
      const durationMatch = request.requested_duration.match(/(\d+)\s*(hour|day)s?/)
      if (durationMatch) {
      ...
      if (memberError) {
        ...
        if (memberError.code === '23505') { // unique_violation
          throw new DatabaseError('User is already a member of this organization', 409)
        }
      ...
      // Deny
      if (!denial_reason) {
        throw new DatabaseError('denial_reason is required when denying a request', 400)
      }
  ...
  if (method === 'DELETE') {
    ...
    if (!isRequester) {
      const { data: membership } = await supabase
        ...
        .eq('organization_id', request.target_org_id)
        .single()
      isTargetOrgAdmin = !!membership && ['owner', 'admin'].includes(membership.role)
    }

    if (!isRequester && !isTargetOrgAdmin && !userContext.isSuperAdmin) {
      throw new DatabaseError('Not authorized to cancel or revoke this request', 403)
    }
    ...
    if (request.status === 'pending' && isRequester) {
      ...
      return createSuccessResponse({ request_id, status: 'cancelled' }, { message: 'Request cancelled.' })
    }
    ...
    if (request.status === 'approved') {
      ...
      return createSuccessResponse({ request_id, status: 'revoked' }, { message: 'Access revoked and temporary membership removed.' })
    }

    throw new DatabaseError(`Cannot revoke a request with status "${request.status}"`, 400)
  }
