-- Add all remaining phone sensor device types
-- Completes the full phone sensor catalog (now 21 phone types total)
-- Previous migration (20260316000000) added 9 phone sensor types;
-- this adds 12 more: GPS sub-types, Battery, Temperature, Humidity,
-- Step Counter, Gravity, Linear Acceleration, Rotation Vector, WiFi Signal

-- Update seed function to include ALL phone sensor types (now 63 total)
...
BEGIN
  INSERT INTO device_types (organization_id, name, description, device_class, unit, lower_normal, upper_normal, lower_alert, upper_alert, precision_digits) VALUES
    ...
    -- Phone Sensors (21) — Issue #528 (complete set)
    -- GPS (5)
    ...
    (org_id, 'Phone GPS Altitude', 'Smartphone GPS altitude above sea level', 'smartphone', 'm', -500.0, 9000.0, -500.0, 9000.0, 1),
    (org_id, 'Phone GPS Speed', 'Smartphone GPS ground speed', 'smartphone', 'm/s', 0.0, 50.0, 0.0, 100.0, 1),
    (org_id, 'Phone GPS Heading', 'Smartphone GPS heading/course (0-360°)', 'smartphone', '°', 0.0, 360.0, 0.0, 360.0, 1),
    -- Motion sensors (3)
    ...
    -- Power (1)
    (org_id, 'Phone Battery Level', 'Smartphone battery state of charge', 'smartphone', '%', 20.0, 100.0, 10.0, 100.0, 0),
    -- Additional environment (2)
    (org_id, 'Phone Temperature', 'Smartphone ambient temperature (Android)', 'smartphone', '°C', 15.0, 35.0, 0.0, 50.0, 1),
    (org_id, 'Phone Humidity', 'Smartphone relative humidity (select Android)', 'smartphone', '%', 30.0, 70.0, 10.0, 90.0, 1),
    -- Derived motion (3)
    (org_id, 'Phone Gravity', 'Smartphone 3-axis gravity vector', 'smartphone', 'm/s²', -10.0, 10.0, -10.0, 10.0, 2),
    (org_id, 'Phone Linear Acceleration', 'Smartphone acceleration minus gravity', 'smartphone', 'm/s²', -20.0, 20.0, -40.0, 40.0, 2),
    (org_id, 'Phone Rotation Vector', 'Smartphone 3D orientation/pose', 'smartphone', '', -1.0, 1.0, -1.0, 1.0, 4),
    -- Activity (1)
    (org_id, 'Phone Step Counter', 'Smartphone pedometer step count', 'smartphone', 'steps', 0.0, 30000.0, 0.0, 50000.0, 0),
    -- Connectivity (1)
    (org_id, 'Phone WiFi Signal', 'Smartphone WiFi RSSI for positioning', 'smartphone', 'dBm', -70.0, -30.0, -90.0, -10.0, 0),
    -- GPS accuracy (1)
    (org_id, 'Phone GPS Accuracy', 'Smartphone GPS horizontal accuracy', 'smartphone', 'm', 0.0, 10.0, 0.0, 100.0, 1);

  RAISE NOTICE '✅ Created 63 standard device types (incl. 21 phone sensors) for organization %', org_id;
END;
$$;

COMMENT ON FUNCTION seed_organization_device_types IS 'Seeds 63 standard IoT device types (including 21 phone sensor types) for an organization';

-- Backfill: insert the 12 NEW phone sensor device types for ALL existing organizations
-- (the original 9 were already backfilled by 20260316000000)
...
CROSS JOIN (VALUES
  ('Phone GPS Altitude',         'Smartphone GPS altitude above sea level',      'smartphone', 'm',     -500.0,  9000.0,  -500.0,  9000.0,  1),
  ('Phone GPS Speed',            'Smartphone GPS ground speed',                  'smartphone', 'm/s',   0.0,     50.0,    0.0,     100.0,   1),
  ('Phone GPS Heading',          'Smartphone GPS heading/course (0-360°)',       'smartphone', '°',     0.0,     360.0,   0.0,     360.0,   1),
  ('Phone Battery Level',        'Smartphone battery state of charge',           'smartphone', '%',     20.0,    100.0,   10.0,    100.0,   0),
  ('Phone Temperature',          'Smartphone ambient temperature (Android)',     'smartphone', '°C',    15.0,    35.0,    0.0,     50.0,    1),
  ('Phone Humidity',             'Smartphone relative humidity (select Android)','smartphone', '%',     30.0,    70.0,    10.0,    90.0,    1),
  ('Phone Gravity',              'Smartphone 3-axis gravity vector',             'smartphone', 'm/s²',  -10.0,   10.0,    -10.0,   10.0,    2),
  ('Phone Linear Acceleration',  'Smartphone acceleration minus gravity',        'smartphone', 'm/s²',  -20.0,   20.0,    -40.0,   40.0,    2),
  ('Phone Rotation Vector',      'Smartphone 3D orientation/pose',               'smartphone', '',      -1.0,    1.0,     -1.0,    1.0,     4),
  ('Phone Step Counter',         'Smartphone pedometer step count',              'smartphone', 'steps', 0.0,     30000.0, 0.0,     50000.0, 0),
  ('Phone WiFi Signal',          'Smartphone WiFi RSSI for positioning',         'smartphone', 'dBm',   -70.0,   -30.0,   -90.0,   -10.0,   0),
  ('Phone GPS Accuracy',         'Smartphone GPS horizontal accuracy',           'smartphone', 'm',     0.0,     10.0,    0.0,     100.0,   1)
