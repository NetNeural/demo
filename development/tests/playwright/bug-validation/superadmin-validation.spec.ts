...
test.describe('Bug Fixes Validation with Superadmin', () => {
  ...
  test('Bug #22: Remember Me Checkbox is visible on login page', async ({ page }) => {
    await page.goto(`${BASE_URL}/auth/login`);
    const rememberMeCheckbox = page.locator('input#remember-me[type="checkbox"]');
    await expect(rememberMeCheckbox).toBeVisible();
    console.log('✅ Bug #22: Remember Me checkbox is visible');
  });
  
  test('Bug #7 & #12: Dashboard shows Alerts and Locations cards', async ({ page }) => {
    ...
    const locationsHeading = page.getByRole('heading', { name: /locations/i }).first();
    await expect(locationsHeading).toBeVisible();
    console.log('✅ Bug #12: Locations card is visible on dashboard');
  });
  
  test('Bug #11: Profile tab loads and save button is visible', async ({ page }) => {
    await page.goto(`${BASE_URL}/dashboard/settings`);
    await page.waitForTimeout(2000);
    
    // Profile tab should be default
    const fullNameInput = page.locator('input[placeholder*="name" i], input#full_name, input[name="full_name"]').first();
  ...
  test('Bug #13 & #16: Preferences tab has theme selector and save button', async ({ page }) => {
    await page.goto(`${BASE_URL}/dashboard/settings`);
  ...
  test('Bug #18, #20, #21: Security tab has password change, sessions, and API keys', async ({ page }) => {
    await page.goto(`${BASE_URL}/dashboard/settings`);
    await page.waitForTimeout(1000);
  ...
  test('Bug #19: Two-Factor Authentication toggle is visible', async ({ page }) => {
    await page.goto(`${BASE_URL}/dashboard/settings`);
    await page.waitForTimeout(1000);
  ...
  test('Bug #6: Organizations page loads with settings available', async ({ page }) => {
    await page.goto(`${BASE_URL}/dashboard/organizations`);
    await page.waitForTimeout(2000);
    
    // Check if organizations page loaded
    const pageHeading = page.getByRole('heading', { name: /organization/i }).first();
    await expect(pageHeading).toBeVisible();
    console.log('✅ Bug #6: Organization management page is accessible');
  });
  
  test('Backend Integration: Dashboard stats load from edge function', async ({ page }) => {
    await page.goto(`${BASE_URL}/dashboard`);
    await page.waitForTimeout(3000);
    
    // Check if dashboard loaded (which means stats API worked)
    const dashboardTitle = page.getByRole('heading', { name: /dashboard/i }).first();
    await expect(dashboardTitle).toBeVisible();
    console.log('✅ Backend: Dashboard stats loaded successfully');
  });
  
  test('Backend Integration: Organizations load from edge function', async ({ page }) => {
    await page.goto(`${BASE_URL}/dashboard/organizations`);
    await page.waitForTimeout(3000);
    
    // If we see organization content, the API call worked
    const pageContent = await page.content();
    const hasOrganizationData = pageContent.includes('NetNeural') || pageContent.includes('organization');
    expect(hasOrganizationData).toBeTruthy();
    console.log('✅ Backend: Organizations API working');
  });
  
  test('Full User Journey: Login → Dashboard → Settings → All Tabs', async ({ page }) => {
