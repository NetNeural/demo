...
async function loginAs(page: Page, email: string, password: string) {
  ...
  if (await mfaInput.isVisible().catch(() => false)) {
    ...
    if (!secret)
      throw new Error(
        'loginAs: TOTP required but no secret on disk — did globalSetup run?'
      )
  ...
  if (page.url().includes('/auth/setup-mfa')) {
    throw new Error(
      'loginAs: landed on /auth/setup-mfa — TOTP factor not verified for this session'
    )
  }
...
test.describe('Customer Signup â€” UI Journey', () => {
  test('signup page loads with plan selection', async ({ page }) => {
    ...
    const planCards = page
      .locator('button')
      .filter({ hasText: /starter|professional|enterprise/i })
  ...
  test('Step 2 â€” password validation rules are enforced', async ({
    page,
  }) => {
    await page.goto('/auth/signup')
    await page.waitForLoadState('load')
  ...
  test('Step 2 â€” fills account details and submits', async ({ page }) => {
    ...
    const nameInput = page
      .locator(
        'input[placeholder*="full name" i], input[placeholder*="your name" i], input[id*="name" i]'
      )
      .first()
    ...
    const orgInput = page
      .locator(
        'input[placeholder*="organization" i], input[placeholder*="company" i], input[id*="org" i]'
      )
      .first()
    ...
    const submitBtn = page
      ...
      .filter({ hasText: /create account|sign up|register|get started/i })
      .last()
    ...
    expect(isEmailStep || isDashboard || isMfaSetup || isChangePassword).toBe(
      true
    )
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
      ...
      if (listErr) {
        console.warn(
          `⚠️  listUsers attempt ${attempt} error: ${listErr.message}`
        )
        continue
      }
      console.log(
        `   listUsers attempt ${attempt}: ${userList?.users?.length ?? 0} total users`
      )
    ...
    if (newUser) {
      ...
      if (isEmailStep) {
        console.log(
          `ℹ️  Email confirmation required — confirmed via admin API (id: ${newUser.id.slice(0, 8)})`
        )
      } else {
        console.log(
          `✅ Signup completed, user found (id: ${newUser.id.slice(0, 8)})`
        )
      }
    } else {
      ...
      console.error(
        `❌ User ${TEST_EMAIL} NOT found after 5 listUsers attempts.`
      )
...
test.describe('Customer Billing â€” DB Records Created', () => {
  ...
  test('organization was created with correct name and subscription_tier', async () => {
    ...
    expect(
      membership,
      `Owner membership should exist for user ${newUserId}`
    ).toBeTruthy()
    ...
    expect(['starter', 'professional', 'enterprise', 'free']).toContain(
  ...
  test('subscription record exists for the organization', async () => {
    ...
    if (sub.stripe_customer_id !== null) {
      ...
      console.log(
        'â„¹ï¸  stripe_customer_id is NULL (expected â€” Stripe not yet configured at signup)'
      )
    }
  })
  ...
  test('billing profile was auto-created by trigger', async () => {
    ...
    const { data: profile, error } = await (supabase as any)
      ...
      .select(
        'id, organization_id, company_name, billing_name, billing_email, address_line1'
      )
      .eq('organization_id', newOrgId)
      .maybeSingle()
    ...
    expect(
      profile,
      `Billing profile should exist for org ${newOrgId}`
    ).toBeTruthy()
  ...
  test('payment method table has no entries (expected â€” customer has not added card yet)', async () => {
    ...
    console.log(
      'â„¹ï¸  No payment methods yet (correct â€” customer must add card to start billing cycle)'
    )
  })
})
...
test.describe('Customer Billing Tab â€” UI Completeness', () => {
  ...
  test('billing tab is accessible from organization settings', async ({
    page,
  }) => {
  ...
  test('billing tab shows subscription plan name', async ({ page }) => {
    ...
    const planName = page
      .locator('text=/starter|professional|enterprise/i')
      .first()
    await expect(planName).toBeVisible({ timeout: 10000 })
  })

  test('billing tab shows payment methods card with empty state', async ({
    page,
  }) => {
    await page.goto('/dashboard/organizations')
    await page.waitForLoadState('load')
  ...
  test('billing tab shows billing contact card with empty state', async ({
    page,
  }) => {
    await page.goto('/dashboard/organizations')
    await page.waitForLoadState('load')
  ...
  test('billing contact can be filled in and saved', async ({ page }) => {
    ...
    const editBtn = page
      .locator(
        '[data-testid="add-billing-contact-btn"], [data-testid="edit-billing-contact-btn"]'
      )
      .first()
    ...
    await page
      .locator('[data-testid="billing-email-input"]')
      .fill(`billing-${RUN_ID}@testcorp.ai`)
    await page
      .locator('[data-testid="company-name-input"]')
      .fill('TestCorp LLC')
    await page
      .locator('[data-testid="address-line1-input"]')
    ...
    await expect(
      page.locator('[data-testid="billing-name-display"]')
    ).toHaveText('Jane Smith')
    await expect(
      page.locator('[data-testid="company-name-display"]')
    ).toHaveText('TestCorp LLC')
    await expect(page.locator('[data-testid="address-display"]')).toHaveText(
      '1 Market Street'
    )
  })

  test('billing contact is persisted in database after save', async ({
    page,
  }) => {
    if (!newOrgId) {
      // Re-fetch org ID
  ...
  test('manage subscription button opens Stripe portal or shows message', async ({
    page,
  }) => {
    await page.goto('/dashboard/organizations')
    await page.waitForLoadState('load')
    ...
    const manageBtn = page
      .locator('button')
      .filter({ hasText: /manage subscription/i })
      .first()
...
test.describe('Admin Billing â€” new customer appears in Customers tab', () => {
  test('new customer appears in admin billing customers list', async ({
    page,
  }) => {
    // Login as admin
  ...
  test('new customer subscription appears in admin subscriptions tab', async ({
    page,
  }) => {
    const adminEmail = process.env.TEST_USER_EMAIL || 'admin@netneural.ai'
    const adminPassword = process.env.TEST_USER_PASSWORD || 'password123'
    ...
    const subsTab = page
      .locator('[role="tab"]')
      .filter({ hasText: /subscriptions/i })
      .first()
