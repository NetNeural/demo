...
test.describe('Bug Hunting - Comprehensive Page Traversal', () => {
  ...
  test('1. Login Page - Check for UI issues', async ({ page }) => {
    ...
    console.log(
      `✓ Remember Me checkbox: ${rememberMeVisible ? 'FOUND' : '❌ MISSING'}`
    )
  ...
  test('2. Dashboard - Check all widgets and data', async ({ page }) => {
    ...
    const alertsCard = page
      .locator('text=Alert')
      .or(page.locator('[data-testid="alerts-card"]'))
    const alertsVisible = (await alertsCard.count()) > 0
    console.log(
      `${alertsVisible ? '✓' : '❌'} Alerts Card: ${alertsVisible ? 'VISIBLE' : 'MISSING'}`
    )
    ...
    console.log(
      `${locationsVisible ? '✓' : '❌'} Locations Card: ${locationsVisible ? 'VISIBLE' : 'MISSING'}`
    )
    ...
    console.log(
      `${devicesVisible ? '✓' : '❌'} Devices Card: ${devicesVisible ? 'VISIBLE' : 'MISSING'}`
    )
    ...
    await page.screenshot({
      path: 'test-results/dashboard-check.png',
      fullPage: true,
    })
  ...
  test('3. Settings - Profile Tab', async ({ page }) => {
    ...
    console.log(
      `${profileVisible ? '✓' : '❌'} Profile Tab: ${profileVisible ? 'VISIBLE' : 'MISSING'}`
    )
    ...
    if (profileVisible) {
      ...
      console.log(
        `${fullNameVisible ? '✓' : '❌'} Full Name field: ${fullNameVisible ? 'VISIBLE' : 'MISSING'}`
      )
      ...
      console.log(
        `${saveVisible ? '✓' : '❌'} Save button: ${saveVisible ? 'VISIBLE' : 'MISSING'}`
      )
    ...
    await page.screenshot({
      path: 'test-results/settings-profile.png',
      fullPage: true,
    })
  ...
  test('4. Settings - Preferences Tab', async ({ page }) => {
    ...
    console.log(
      `${preferencesVisible ? '✓' : '❌'} Preferences Tab: ${preferencesVisible ? 'VISIBLE' : 'MISSING'}`
    )
    ...
    if (preferencesVisible) {
      ...
      const themeToggle = page
        .locator('text=Theme')
        .or(page.locator('text=Dark Mode'))
      const themeVisible = (await themeToggle.count()) > 0
      console.log(
        `${themeVisible ? '✓' : '❌'} Theme toggle: ${themeVisible ? 'VISIBLE' : 'MISSING'}`
      )
      ...
      const saveVisible = (await saveButton.count()) > 0
      console.log(
        `${saveVisible ? '✓' : '❌'} Save Preferences button: ${saveVisible ? 'VISIBLE' : 'MISSING'}`
      )
    ...
    await page.screenshot({
      path: 'test-results/settings-preferences.png',
      fullPage: true,
    })
  ...
  test('5. Settings - Security Tab', async ({ page }) => {
    ...
    console.log(
      `${securityVisible ? '✓' : '❌'} Security Tab: ${securityVisible ? 'VISIBLE' : 'MISSING'}`
    )
    ...
    if (securityVisible) {
      ...
      const passwordSection = page.locator(
        'text=/change password|current password/i'
      )
      const passwordVisible = (await passwordSection.count()) > 0
      console.log(
        `${passwordVisible ? '✓' : '❌'} Password Change section: ${passwordVisible ? 'VISIBLE' : 'MISSING'}`
      )
      ...
      console.log(
        `${twoFAVisible ? '✓' : '❌'} 2FA section: ${twoFAVisible ? 'VISIBLE' : 'MISSING'}`
      )
      ...
      console.log(
        `${sessionsVisible ? '✓' : '❌'} Active Sessions section: ${sessionsVisible ? 'VISIBLE' : 'MISSING'}`
      )
      ...
      console.log(
        `${apiKeysVisible ? '✓' : '❌'} API Keys section: ${apiKeysVisible ? 'VISIBLE' : 'MISSING'}`
      )
    }

    await page.screenshot({
      path: 'test-results/settings-security.png',
      fullPage: true,
    })
  ...
  test('6. Organizations Page', async ({ page }) => {
    ...
    const orgCards = page
      ...
      .or(page.locator('text=/organization/i'))
    ...
    console.log(
      `${addButtonVisible ? '✓' : '❌'} Add/Create button: ${addButtonVisible ? 'VISIBLE' : 'MISSING'}`
    )

    await page.screenshot({
      path: 'test-results/organizations.png',
      fullPage: true,
    })
  ...
  test('7. Organization Detail Page', async ({ page }) => {
    ...
    const firstOrg = page
      .locator('a, button')
      .filter({ hasText: /organization|view|details/i })
      .first()
    ...
    if (orgVisible) {
      ...
      for (const tab of tabs) {
        ...
        console.log(
          `${tabVisible ? '✓' : '❌'} ${tab} tab: ${tabVisible ? 'VISIBLE' : 'MISSING'}`
        )
      }

      await page.screenshot({
        path: 'test-results/organization-detail.png',
        fullPage: true,
      })
  ...
  test('8. Devices Page', async ({ page }) => {
    ...
    const deviceItems = page
      .locator('[data-testid="device-item"]')
      .or(page.locator('text=/device/i'))
    ...
    const addButtonVisible = (await addButton.count()) > 0
    console.log(
      `${addButtonVisible ? '✓' : '❌'} Add Device button: ${addButtonVisible ? 'VISIBLE' : 'MISSING'}`
    )
  ...
  test('9. Alerts Page', async ({ page }) => {
    ...
    const alertItems = page
      ...
      .or(page.locator('text=/alert|critical|warning/i'))
  ...
  test('12. Check for accessibility issues', async ({ page }) => {
    ...
    console.log(
      `✓ Found ${h1s} H1 heading(s) ${h1s !== 1 ? '(should typically be 1)' : ''}`
    )
  })
})
