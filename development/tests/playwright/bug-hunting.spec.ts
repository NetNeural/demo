...
test.describe('Bug Hunting - Page Traversal', () => {
  ...
  test.describe('Network Error Handling', () => {
    test('app handles failed API requests gracefully', async ({ page }) => {
      ...
      const errorOverlay = page.locator(
        '#__next-build-error, [class*="error-overlay"]'
      )
  ...
  test.describe('Responsive Layout', () => {
    test('mobile viewport shows hamburger or collapses sidebar', async ({
      page,
    }) => {
      ...
      const menuBtn = page.locator(
        'button[aria-label*="menu"], button[aria-label*="Menu"], [class*="hamburger"], [class*="mobile-menu"]'
      )
      ...
      console.log(
        `Mobile: menu button visible=${menuVisible}, sidebar visible=${sidebarVisible}`
      )
    })
  })
