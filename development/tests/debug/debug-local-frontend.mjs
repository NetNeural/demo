...
;(async () => {
  ...
  page.on('request', (request) => {
    ...
    if (
      url.includes('54321/functions/v1') ||
  ...
  try {
    ...
    const noOrgMessage = await page
      .locator('text=No organization selected')
      .count()
    console.log(
      'No organization message:',
      noOrgMessage > 0 ? 'YES - Organization not set!' : 'No'
    )

    const noDevicesMessage = await page.locator('text=No devices found').count()
    console.log(
      'No devices found message:',
      noDevicesMessage > 0 ? 'YES' : 'No'
    )
    ...
    const pageText = await page.evaluate(() => {
      ...
      return mainContent
        ? mainContent.innerText.substring(0, 500)
    ...
    const deviceNames = await page.evaluate(() => {
      ...
      const matches = text.match(
        /Temperature Sensor|Humidity Monitor|HVAC|Motion Detector/gi
      )
      return matches || []
    })
    ...
    console.log('\n📡 API Calls made:')
    console.log(
      apiCalls.length > 0 ? apiCalls : 'No device-related API calls detected'
    )
