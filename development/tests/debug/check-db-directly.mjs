  ...
  try {
    ...
    const dbCheck = await page.evaluate(async () => {
      ...
      try {
        ...
        const allDevicesResp = await fetch('https://bldojxpockljyivldxwf.supabase.co/rest/v1/devices?select=*&limit=100', {
          headers
        });
        const allDevices = await allDevicesResp.json();
        
        // 2. Check devices for NetNeural org
        const netNeuralDevicesResp = await fetch('https://bldojxpockljyivldxwf.supabase.co/rest/v1/devices?organization_id=eq.00000000-0000-0000-0000-000000000001&select=*', {
          headers
        });
        const netNeuralDevices = await netNeuralDevicesResp.json();
        
        // 3. Check devices for Tes Org
        const tesOrgDevicesResp = await fetch('https://bldojxpockljyivldxwf.supabase.co/rest/v1/devices?organization_id=eq.11ec1e5c-a9df-4313-8ca3-15675f35f673&select=*', {
          headers
        });
        const tesOrgDevices = await tesOrgDevicesResp.json();
        
        // 4. Check user's organization_id from users table
        const userResp = await fetch('https://bldojxpockljyivldxwf.supabase.co/rest/v1/users?id=eq.d17ea1bd-a26f-44fa-93fb-fd2fbffeb9b0&select=organization_id,role', {
          headers
        });
        const userRecord = await userResp.json();
        
        // 5. Check if devices table has any records at all
        const deviceCountResp = await fetch('https://bldojxpockljyivldxwf.supabase.co/rest/v1/devices?select=count', {
          headers: {
            ...headers,
            'Prefer': 'count=exact'
          }
        ...
        return {
          allDevices: {
            ...
            data: Array.isArray(allDevices) ? allDevices.slice(0, 3) : allDevices
          },
          netNeuralDevices: {
            count: Array.isArray(netNeuralDevices) ? netNeuralDevices.length : 0,
            data: Array.isArray(netNeuralDevices) ? netNeuralDevices : netNeuralDevices
          },
          tesOrgDevices: {
    ...
    if (dbCheck.error) {
      ...
      if (dbCheck.allDevices.count > 0) {
        console.log('   Sample:', JSON.stringify(dbCheck.allDevices.data, null, 2));
      } else {
        console.log('   Response:', JSON.stringify(dbCheck.allDevices.data, null, 2));
      }
      ...
      if (dbCheck.tesOrgDevices.count > 0) {
        console.log('   Sample:', JSON.stringify(dbCheck.tesOrgDevices.data, null, 2));
      } else {
        console.log('   Response:', JSON.stringify(dbCheck.tesOrgDevices.data, null, 2));
      }
    ...
    if (dbCheck.allDevices?.count === 0) {
      ...
      console.log('⚠️  Complex issue - check RLS policies and organization context');
    }
