  ...
  try {
    ...
    const userData = await page.evaluate(async () => {
      ...
      try {
        ...
        // Get organizations
        const orgsResponse = await fetch('https://bldojxpockljyivldxwf.supabase.co/functions/v1/organizations', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        ...
        if (userRecord[0]?.organization_id) {
          const devicesResponse = await fetch(`https://bldojxpockljyivldxwf.supabase.co/functions/v1/devices?organization_id=${userRecord[0].organization_id}`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
