...
async function diagnoseProduction() {
  ...
  try {
    ...
    const navItems = await page.locator('nav a, nav button, [role="navigation"] a, [role="navigation"] button').allTextContents();
    ...
    const hasOrganizations = navItems.some(item => item.includes('Organization'));
    ...
    const orgResponse = await page.evaluate(async () => {
      try {
        const response = await fetch('https://bldojxpockljyivldxwf.supabase.co/functions/v1/organizations', {
          headers: {
    ...
    const corsTest = await page.evaluate(() => {
      ...
      const consoleErrors = window.localStorage.getItem('_debug_cors_errors') || '[]';
      
      return {
        hasCorsErrors: consoleErrors.includes('CORS') || consoleErrors.includes('cors'),
    ...
    const supabaseStatus = await page.evaluate(() => {
      const authToken = localStorage.getItem('sb-bldojxpockljyivldxwf-auth-token');
    ...
    console.log(`Auth token present: ${supabaseStatus.hasAuthToken ? '✅' : '❌'}`);
    ...
    if (failedRequests.length > 0) {
      ...
      uniqueUrls.forEach(url => {
        ...
        console.log(`❌ ${url}`);
        console.log(`   Status: ${requests[0].status}, Count: ${requests.length}`);
      });
      console.log('');
    }
