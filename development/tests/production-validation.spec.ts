...
test.describe('Production Validation - demo.netneural.ai', () => {
  ...
  test('Bug #22: Remember Me checkbox visible on login', async ({ page }) => {
    ...
    const rememberMeCheckbox = page.locator('input[type="checkbox"]').filter({ hasText: /remember me/i }).or(
      page.locator('label:has-text("Remember Me")').locator('input[type="checkbox"]')
    );
  ...
  test('Login functionality works', async ({ page }) => {
    ...
    // Click login button
    const loginButton = page.locator('button[type="submit"]').filter({ hasText: /sign in|log in|login/i }).first();
  ...
  test('Dashboard loads with widgets', async ({ page }) => {
    ...
    const alertsCard = page.locator('text=Alerts').or(page.locator('text=Recent Alerts'));
    ...
    const locationsCard = page.locator('text=Locations').or(page.locator('text=Location'));
  ...
  test('Settings page accessible', async ({ page }) => {
    ...
    const profileTab = page.locator('text=Profile').or(page.locator('[role="tab"]:has-text("Profile")'));
    const preferencesTab = page.locator('text=Preferences').or(page.locator('[role="tab"]:has-text("Preferences")'));
    const securityTab = page.locator('text=Security').or(page.locator('[role="tab"]:has-text("Security")'));
    ...
    const preferencesExists = await preferencesTab.first().isVisible().catch(() => false);
    const securityExists = await securityTab.first().isVisible().catch(() => false);
  ...
  test('Organizations page accessible', async ({ page }) => {
    ...
    // Check page loaded
    const organizationsHeading = page.locator('h1, h2').filter({ hasText: /organization/i });
  ...
  test('Check deployment version', async ({ page }) => {
    ...
    const hasLocationCard = pageContent.includes('LocationsCard') || pageContent.includes('Locations');
    const hasRememberMe = pageContent.includes('Remember Me') || pageContent.includes('remember');
  ...
  test('Responsive design check', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      console.log(`✅ ${viewport.name} view (${viewport.width}x${viewport.height}) rendered successfully`);
    }
  });
