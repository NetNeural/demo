...
;(async () => {
  ...
  try {
    ...
    const switcherButton = await page
      .locator('button:has-text("NetNeural")')
      .first()
    if ((await switcherButton.count()) > 0) {
      ...
      const tesOrgOption = await page
        .locator('[role="menuitem"]:has-text("Tes Org")')
        .first()
      if ((await tesOrgOption.count()) > 0) {
        ...
        if (apiCalls.length > 1) {
          ...
          if (lastCall.orgId === '11ec1e5c-a9df-4313-8ca3-15675f35f673') {
            ...
            lastCall.orgId === '00000000-0000-0000-0000-000000000001'
          ) {
        ...
        const deviceCards = await page
          .locator('[data-device-id], .device-card, .device-item')
          .count()
        ...
        const hasDeviceText =
          pageText?.includes('HVAC') || pageText?.includes('Test Device')
