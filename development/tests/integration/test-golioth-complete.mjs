...
(async () => {
  ...
  try {
    ...
    await page.goto(`${LOCAL_URL}/dashboard/integrations`, { waitUntil: 'networkidle', timeout: 30000 });
    ...
    if (existingIntegrations > 0) {
      ...
      const addButton = page.locator('button:has-text("Add Integration"), button:has-text("New Integration")').first();
      ...
      await page.fill('input[id="integration-name"], input[placeholder*="name" i]', 'NetNeural Cellular');
      await page.waitForTimeout(500);
      
      await page.fill('input[id="api-key"], input[placeholder*="api key" i]', GOLIOTH_API_KEY);
      await page.waitForTimeout(500);
      
      await page.fill('input[id="project-id"], input[placeholder*="project" i]', GOLIOTH_PROJECT_ID);
      await page.waitForTimeout(500);
      
      // Base URL might be auto-filled
      const baseUrlInput = page.locator('input[id="base-url"], input[placeholder*="base url" i]').first();
      ...
      const syncTab = page.locator('text=/sync.*settings?/i, [role="tab"]:has-text("Sync")').first();
      ...
      if (tabCount > 0) {
        ...
        const syncToggle = page.locator('input[type="checkbox"]:near(:text("Automatic Sync")), input[type="checkbox"]:near(:text("Enable Sync"))').first();
        ...
        const directionSelect = page.locator('select:near(:text("Direction")), [role="combobox"]:near(:text("Direction"))').first();
        const selectCount = await directionSelect.count();
        
        if (selectCount > 0) {
          console.log('Setting sync direction to Import (Golioth → NetNeural)...');
      ...
      const saveButton = page.locator('button:has-text("Save Configuration"), button:has-text("Save"), button:has-text("Create")').first();
      await saveButton.click();
      await page.waitForTimeout(3000);
      
      // Check for success message
      const saveSuccess = await page.locator('text=/saved|created|success/i').count();
      if (saveSuccess > 0) {
    ...
    const syncButton = page.locator('button:has-text("Sync Now"), button:has-text("Import"), button:has-text("Sync Devices")').first();
    ...
    if (syncButtonCount > 0) {
      ...
      const syncSuccess = await page.locator('text=/synced|imported|success/i').count();
    ...
    const deviceNames = await page.evaluate(() => {
      ...
      const matches = text.match(/Temperature|Humidity|HVAC|Motion|Sensor|Gateway|Device/gi);
      return matches || [];
    });
    ...
    const deviceLines = pageContent.split('\n').filter(line => {
      return line.match(/sensor|gateway|device|hvac|temperature|humidity|motion/i);
    });
    ...
    await page.screenshot({ path: 'golioth-integration-complete.png', fullPage: true });
