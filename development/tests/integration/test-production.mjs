...
async function testProduction() {
  ...
  try {
    ...
    const alertCount = await page.locator('div[role="alert"], .alert-item, table tbody tr').count();
    ...
    if (await integrationsTab.isVisible()) {
      ...
      const integrationCards = await page.locator('[class*="integration"], [class*="card"]').count();
      ...
      issues.push({ type: 'Missing Element', message: 'Integrations tab not found' });
    }
    ...
    await page.screenshot({ path: 'production-test-screenshot.png', fullPage: false });
    ...
    try {
      await page.screenshot({ path: 'production-error-screenshot.png', fullPage: false });
