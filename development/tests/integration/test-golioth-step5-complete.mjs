...
(async () => {
  ...
  page.on('response', async response => {
    ...
    if (url.includes('/functions/v1/device-sync') || url.includes('/functions/v1/integrations')) {
      try {
  ...
  try {
    ...
    const syncNowButton = await page.locator('button:has-text("Sync Now"), button:has-text("Import Now")').count();
    if (syncNowButton > 0) {
    ...
    const deviceCount = await page.evaluate(() => {
      ...
      const matches = text.match(/C252400001|nn-gateway-01|C253700003|C253700002/gi);
    ...
    if (deviceCount > 0) {
      ...
      ['C252400001', 'nn-gateway-01', 'C253700003', 'C253700002'].forEach(name => {
