  ...
  try {
    ...
    try {
      ...
      if (integrationsData.integrations) {
        console.log(
          `Found ${integrationsData.integrations.length} integration(s):\n`
        )
        ...
        // Find Golioth
        const golioth = integrationsData.integrations.find(
          (i) => i.type === 'golioth' || i.integrationType === 'golioth'
        )
        ...
        if (golioth) {
          ...
          const syncResponse = await fetch(
            `${SUPABASE_URL}/functions/v1/device-sync`,
            {
              method: 'POST',
              headers: {
                Authorization: `Bearer ${access_token}`,
