...
(async () => {
  ...
  page.on('response', async response => {
    ...
    if (url.includes('/functions/v1/devices') || url.includes('/rest/v1/devices')) {
      try {
    ...
    if (apiResponses.length > 0) {
      ...
      if (resp.body.success && resp.body.data) {
        ...
        if (devices.length > 0) {
          ...
          console.log('  - batteryLevel:', device.batteryLevel !== undefined ? '✓' : '✗');
          console.log('  - isExternallyManaged:', device.isExternallyManaged !== undefined ? '✓' : '✗');
        }
      } else {
      ...
      const relevantRequests = allRequests.filter(u => 
        u.includes('devices') || u.includes('functions') || u.includes('dashboard')
      );
