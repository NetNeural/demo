...
(async () => {
  ...
  try {
    ...
    if (syncButton) {
      ...
      const success = await page.locator('text=/synced|imported|success|complete/i').count();
      ...
      console.log('⚠️  Sync button not found - checking for Configure option...\n');
      
      // Try to open the integration config
      const configButton = await page.locator('button:has-text("Configure")').count();
      if (configButton > 0) {
        ...
        const syncTab = await page.locator('[role="tab"]:has-text("Sync"), button:has-text("Sync Settings")').count();
        if (syncTab > 0) {
          console.log('Found Sync tab, clicking...');
          await page.locator('[role="tab"]:has-text("Sync"), button:has-text("Sync Settings")').first().click();
          await page.waitForTimeout(1000);
        }
        
        // Now look for sync button in dialog
        const dialogSyncButton = await page.locator('button:has-text("Sync Now"), button:has-text("Import Now")').count();
        if (dialogSyncButton > 0) {
          console.log('Clicking sync button in dialog...');
          await page.click('button:has-text("Sync Now"), button:has-text("Import Now")');
    ...
    const syncCalls = apiCalls.filter(c => 
    ...
    if (syncCalls.length > 0) {
      syncCalls.forEach((call, idx) => {
        console.log(`  ${idx + 1}. ${call.endpoint} - ${call.status} ${call.success ? '✅' : '❌'}`);
        if (call.body) {
          if (call.body.devices_succeeded !== undefined) {
            console.log(`     → Devices succeeded: ${call.body.devices_succeeded}`);
