  ...
  try {
    ...
    try {
      ...
      const orgSwitcher = page
        .locator(
          '[class*="organization"], button:has-text("NetNeural"), button:has-text("Test Org")'
        )
        .first()
      ...
      if (await orgSwitcher.isVisible({ timeout: 2000 })) {
        ...
        const testOrg = page
          .locator('text=/Test.*Org|NetNeural.*Demo|Demo.*Organization/i')
          .first()
        if (await testOrg.isVisible({ timeout: 2000 })) {
          ...
          console.log(
            '⚠️  Could not find Test Org option - clicking outside to close'
          )
    ...
    const uniqueLinks = [
      ...new Set(navLinks.filter((link) => link.trim().length > 0)),
    ]
    ...
    try {
      ...
      const devicesLink = page
        .locator('a:has-text("Devices"), button:has-text("Devices")')
        .first()
    ...
    const deviceElements = {
      ...
      hasRows: await page
        .locator('table tbody tr, [data-testid="device-item"]')
        .count(),
      hasEmptyState:
        (await page.locator('text=/no devices/i, text=/empty/i').count()) > 0,
    }
    ...
    try {
      const alertsLink = page
        .locator('a:has-text("Alerts"), button:has-text("Alerts")')
        .first()
    ...
    const alertCount = await page
      .locator('[data-testid="alert-item"], table tbody tr, .alert')
      .count()
    ...
    try {
      const orgsLink = page
        .locator(
          'a:has-text("Organizations"), button:has-text("Organizations")'
        )
        .first()
      ...
      const tabs = await page
        .locator(
          '[role="tab"], button:has-text("Integrations"), button:has-text("Devices")'
        )
        .count()
