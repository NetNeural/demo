...
(async () => {
  ...
  try {
    ...
    const response1 = await page.evaluate(async ({ orgId, token }) => {
    ...
    const response2 = await page.evaluate(async ({ orgId, token }) => {
      const resp = await fetch(
        `https://bldojxpockljyivldxwf.supabase.co/functions/v1/devices?organization_id=${orgId}`,
        {
          headers: {
    ...
    const response3 = await page.evaluate(async ({ token }) => {
      const resp = await fetch(
