...
async function compareEnvironments() {
  ...
  const prodResults = await checkPageAvailability(
    browser,
    PRODUCTION_URL,
    TEST_PAGES
  )
  ...
  const stagingResults = await checkPageAvailability(
    browser,
    STAGING_URL,
    TEST_PAGES
  )
  ...
  for (const path of TEST_PAGES) {
    ...
    const status =
      prod.accessible && stage.accessible
        ? '✅'
        : prod.accessible && !stage.accessible
          ? '❌ MISSING'
          : !prod.accessible && stage.accessible
  ...
  if (missing.length > 0) {
    ...
    for (const item of missing) {
      ...
      console.log(
        `      Staging: ${item.stage.status} (${item.stage.error || 'Not accessible'})`
      )
    }
  }
