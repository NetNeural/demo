...
test.describe('Bug Fix Screenshots', () => {
  ...
  test('Capture all bug fix screenshots', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/bug-22-remember-me.png', fullPage: true });
    ...
    await page.screenshot({ path: 'screenshots/bug-07-12-dashboard-alerts-locations.png', fullPage: true });
    
    // 3. Navigate to Settings - Profile Tab (Bug #11, #8-10)
    await page.goto(`${BASE_URL}/dashboard/settings`);
    await page.waitForTimeout(2000);
    await page.screenshot({ path: 'screenshots/bug-11-profile-save.png', fullPage: true });
    
    // 4. Switch to Preferences Tab (Bug #13, #16, #14-15, #17)
    await page.click('button:has-text("Preferences")');
    await page.waitForTimeout(1000);
    await page.screenshot({ path: 'screenshots/bug-13-16-preferences-theme.png', fullPage: true });
    
    // 5. Switch to Security Tab (Bug #18, #20, #21, #19)
    await page.click('button:has-text("Security")');
    await page.waitForTimeout(1000);
    await page.screenshot({ path: 'screenshots/bug-18-20-21-security.png', fullPage: true });
    
    // Scroll down to see 2FA section (Bug #19)
    await page.evaluate(() => window.scrollTo(0, 800));
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'screenshots/bug-19-2fa-section.png', fullPage: false });
    ...
    if (await orgCard.isVisible()) {
      await orgCard.click();
      await page.waitForTimeout(2000);
      await page.screenshot({ path: 'screenshots/bug-06-organization-settings.png', fullPage: true });
      
      // 7. Organization Devices Tab (Bug #1)
      await page.click('button:has-text("Devices")');
      await page.waitForTimeout(1000);
      await page.screenshot({ path: 'screenshots/bug-01-add-device-button.png', fullPage: true });
      
      // 8. Organization Members Tab (Bug #2)
      await page.click('button:has-text("Members")');
      await page.waitForTimeout(1000);
      await page.screenshot({ path: 'screenshots/bug-02-add-member-button.png', fullPage: true });
      
      // 9. Organization Locations Tab (Bug #3)
      await page.click('button:has-text("Locations")');
      await page.waitForTimeout(1000);
      await page.screenshot({ path: 'screenshots/bug-03-add-location-button.png', fullPage: true });
      
      // 10. Organization Integrations Tab (Bug #4)
      await page.click('button:has-text("Integrations")');
      await page.waitForTimeout(1000);
      await page.screenshot({ path: 'screenshots/bug-04-add-integration-button.png', fullPage: true });
      
      // 11. Organization Alerts Tab (Bug #5)
      await page.click('button:has-text("Alerts")');
      await page.waitForTimeout(1000);
      await page.screenshot({ path: 'screenshots/bug-05-view-alerts-button.png', fullPage: true });
    }
