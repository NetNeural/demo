...
test.describe('Device Management', () => {
  ...
  test.describe('Device Listing', () => {
    ...
    test('should load devices page successfully', async ({ page }) => {
      ...
      await expect(page.locator('text=/devices|device list/i').first()).toBeVisible()
      
      // Check for devices list or empty state
      const devicesList = page.locator('[data-testid="devices-list"]').or(
        page.locator('text=/no devices|add device/i')
      )
    ...
    test('should display device cards with basic info', async ({ page }) => {
      ...
      const deviceCards = page.locator('[data-testid="device-card"]').or(
        page.locator('[class*="device-card"]')
      )
    ...
    test('should show device count', async ({ page }) => {
      ...
      const deviceCount = page.locator('text=/\\d+ devices?/i').or(
        page.locator('[data-testid="device-count"]')
      )
  ...
  test.describe('Device Filtering', () => {
    ...
    test('should filter by device type (sensor/gateway)', async ({ page }) => {
      ...
      const typeFilter = page.locator('[data-testid="device-type-filter"]').or(
    ...
    test('should filter by device status (online/offline/warning)', async ({ page }) => {
      await page.waitForTimeout(2000)
      
      // Look for status filter
      const statusFilter = page.locator('[data-testid="device-status-filter"]').or(
  ...
  test.describe('Device Sorting', () => {
    ...
    test('should sort devices by name', async ({ page }) => {
      ...
      const sortButton = page.locator('[data-testid="sort-devices"]').or(
    ...
    test('should sort devices by status', async ({ page }) => {
      ...
      if (await sortButton.isVisible({ timeout: 5000 })) {
        ...
        const statusOption = page.locator('text=/sort.*status|status.*sort/i').first()
    ...
    test('should sort devices by battery level', async ({ page }) => {
      ...
      if (await sortButton.isVisible({ timeout: 5000 })) {
        ...
        const batteryOption = page.locator('text=/sort.*battery|battery.*sort/i').first()
  ...
  test.describe('Temperature Unit Toggle', () => {
    ...
    test('should toggle between Fahrenheit and Celsius', async ({ page }) => {
      ...
      const tempToggle = page.locator('[data-testid="temp-unit-toggle"]').or(
        page.locator('button:has-text("°F"), button:has-text("°C")')
      )
  ...
  test.describe('Device Details', () => {
    ...
    test('should open device details on click', async ({ page }) => {
      ...
      const deviceCard = page.locator('[data-testid="device-card"]').first().or(
        page.locator('[class*="device-card"]').first()
      )
      ...
      if (await deviceCard.isVisible({ timeout: 5000 })) {
        ...
        const detailsView = page.locator('[data-testid="device-details"]').or(
          page.locator('text=/device.*details|details.*device/i')
        )
    ...
    test('should display device telemetry', async ({ page }) => {
      ...
      if (await deviceCard.isVisible({ timeout: 5000 })) {
        ...
        const telemetryData = page.locator('text=/temperature|battery|humidity|pressure/i')
  ...
  test.describe('Device Sync', () => {
    ...
    test('should display sync button for Golioth devices', async ({ page }) => {
      ...
      const syncButton = page.locator('[data-testid="sync-device"]').or(
  ...
  test.describe('CSV Export', () => {
    ...
    test('should display CSV export button', async ({ page }) => {
      ...
      const exportButton = page.locator('[data-testid="export-devices"]').or(
        page.locator('button:has-text("Export"), button:has-text("CSV")')
      )
  ...
  test.describe('Pagination', () => {
    ...
    test('should paginate when more than 25 devices', async ({ page }) => {
      ...
      // Check device count
      const deviceCountText = await page.locator('text=/\\d+ devices?/i').first().textContent()
      const deviceCount = parseInt(deviceCountText?.match(/\d+/)?.[0] || '0')
      
      if (deviceCount > 25) {
        // Look for pagination controls
        const paginationNext = page.locator('[data-testid="pagination-next"]').or(
  ...
  test.describe('Error Handling', () => {
    ...
    test('should show retry button on error', async ({ page }) => {
      ...
      if (await retryButton.first().isVisible({ timeout: 5000 })) {
        ...
        const devicesContent = page.locator('[data-testid="devices-list"]').or(
          page.locator('text=/devices/i')
        )
