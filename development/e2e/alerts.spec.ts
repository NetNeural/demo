...
test.describe('Alert Management', () => {
  ...
  test.describe('Alert Listing', () => {
    ...
    test('should load alerts page successfully', async ({ page }) => {
      ...
      await expect(page.locator('text=/alerts|alert list/i').first()).toBeVisible()
      
      // Check for alerts list or empty state
      const alertsList = page.locator('[data-testid="alerts-list"]').or(
        page.locator('text=/no alerts|all clear/i')
      )
    ...
    test('should display alert count', async ({ page }) => {
      ...
      const alertCount = page.locator('text=/\\d+ alerts?/i').or(
        page.locator('[data-testid="alert-count"]')
      )
    ...
    test('should show alerts with severity indicators', async ({ page }) => {
      ...
      const alertCards = page.locator('[data-testid="alert-card"]').or(
        page.locator('[class*="alert-card"]')
      )
      ...
      if (count > 0) {
        ...
        const severityBadge = firstAlert.locator('text=/critical|high|medium|low/i')
  ...
  test.describe('Alert Filtering', () => {
    ...
    test('should filter by tab (all/unacknowledged/connectivity/security/environmental)', async ({ page }) => {
      await page.waitForTimeout(2000)
      
      const tabs = ['All', 'Unacknowledged', 'Connectivity', 'Security', 'Environmental']
    ...
    test('should filter by severity (critical/high/medium/low)', async ({ page }) => {
      await page.waitForTimeout(2000)
    ...
    test('should filter by category (temperature/battery/connectivity)', async ({ page }) => {
      await page.waitForTimeout(2000)
      
      // Look for category filter
      const categoryFilter = page.locator('[data-testid="category-filter"]').or(
        page.locator('button:has-text("Temperature"), button:has-text("Battery")')
      )
  ...
  test.describe('View Modes', () => {
    ...
    test('should toggle between card and table view', async ({ page }) => {
      ...
      const cardViewButton = page.locator('button:has-text("Cards"), [data-testid="view-cards"]').first()
      const tableViewButton = page.locator('button:has-text("Table"), [data-testid="view-table"]').first()
  ...
  test.describe('Alert Details', () => {
    ...
    test('should open alert details dialog on click', async ({ page }) => {
      ...
      const alertCard = page.locator('[data-testid="alert-card"]').first().or(
        page.locator('[class*="alert-card"]').first()
      )
      ...
      if (await alertCard.isVisible({ timeout: 5000 })) {
        ...
        const detailsDialog = page.locator('[data-testid="alert-details"]').or(
    ...
    test('should display comprehensive alert information', async ({ page }) => {
      ...
      if (await alertCard.isVisible({ timeout: 5000 })) {
        ...
        if (await detailsDialog.isVisible()) {
          ...
          const currentValue = detailsDialog.locator('text=/current.*value|value.*current/i')
          ...
          await expect(currentValue.or(threshold).or(deviceInfo).first()).toBeVisible()
        }
      }
    })
  ...
  test.describe('Alert Acknowledgment', () => {
    ...
    test('should acknowledge single alert', async ({ page }) => {
      ...
      const unacknowledgedAlerts = page.locator('[data-testid="alert-card"]').or(
        page.locator('[class*="alert-card"]')
      )
      ...
      if (count > 0) {
        ...
        const acknowledgeButton = firstAlert.locator('button:has-text("Acknowledge")').or(
          page.locator('[data-testid="acknowledge-alert"]')
        ).first()
    ...
    test('should show acknowledgment types (acknowledged/dismissed/resolved/false-positive)', async ({ page }) => {
      await page.waitForTimeout(2000)
      ...
      if (await alertCard.isVisible({ timeout: 5000 })) {
        ...
        if (await acknowledgeButton.isVisible({ timeout: 5000 })) {
          ...
          const typeOptions = page.locator('text=/acknowledged|dismissed|resolved|false positive/i')
    ...
    test('should add notes to acknowledgment', async ({ page }) => {
      ...
      if (await alertCard.isVisible({ timeout: 5000 })) {
        ...
        if (await acknowledgeButton.isVisible({ timeout: 5000 })) {
          ...
          const notesInput = page.locator('textarea[placeholder*="notes"], textarea[placeholder*="comment"]').first()
          
          if (await notesInput.isVisible({ timeout: 5000 })) {
            await notesInput.fill('This was a sensor malfunction, replaced sensor')
    ...
    test('should track who acknowledged alerts', async ({ page }) => {
      ...
      const acknowledgedAlert = page.locator('text=/acknowledged by|acknowledged at/i').first()
  ...
  test.describe('Bulk Acknowledgment', () => {
    ...
    test('should select multiple alerts', async ({ page }) => {
      ...
      const alertCheckboxes = page.locator('input[type="checkbox"]').or(
        page.locator('[data-testid="alert-checkbox"]')
      )
      ...
      if (count >= 2) {
        ...
        const bulkToolbar = page.locator('[data-testid="bulk-actions"]').or(
          page.locator('text=/\\d+ selected/i')
        )
    ...
    test('should bulk acknowledge selected alerts', async ({ page }) => {
      ...
      if (count >= 2) {
        ...
        const bulkAcknowledgeButton = page.locator('button:has-text("Acknowledge Selected")').or(
          page.locator('[data-testid="bulk-acknowledge"]')
        ).first()
  ...
  test.describe('Grouped Alerts', () => {
    ...
    test('should group alerts by device', async ({ page }) => {
      ...
      const groupedAlerts = page.locator('[data-testid="grouped-alerts"]').or(
        page.locator('text=/\\d+ alerts from/i')
      )
    ...
    test('should expand grouped alerts', async ({ page }) => {
      ...
      const groupedAlerts = page.locator('[data-testid="grouped-alerts"]').first()
  ...
  test.describe('Alert Thresholds', () => {
    ...
    test('should display threshold configuration page', async ({ page }) => {
      ...
      const thresholds = page.locator('[data-testid="thresholds-list"]').or(
        page.locator('text=/threshold|configure/i')
      )
    ...
    test('should create new threshold', async ({ page }) => {
      ...
      const createButton = page.locator('button:has-text("Create"), button:has-text("Add Threshold")').first()
  ...
  test.describe('Email Notifications', () => {
    ...
    test('should show email notification settings', async ({ page }) => {
      ...
      const emailSettings = page.locator('text=/email.*notification|notification.*email/i').or(
  ...
  test.describe('Real-time Updates', () => {
    ...
    test('should show real-time alert updates via WebSocket', async ({ page }) => {
      await page.waitForTimeout(2000)
      
      // Get initial alert count
      const initialCountText = await page.locator('text=/\\d+ alerts?/i').first().textContent() || '0'
      ...
      // Get updated count
      const updatedCountText = await page.locator('text=/\\d+ alerts?/i').first().textContent() || '0'
  ...
  test.describe('Empty States', () => {
    ...
    test('should show helpful message when no alerts', async ({ page }) => {
      ...
      const emptyState = page.locator('text=/no alerts|all clear|no active alerts/i')
  ...
  test.describe('Audit Log', () => {
    ...
    test('should track alert acknowledgments in audit log', async ({ page }) => {
      // Navigate to audit log or user actions page
      await page.goto('/dashboard/audit')
      await page.waitForLoadState('networkidle')
      
      // Look for audit log entries
      const auditLog = page.locator('[data-testid="audit-log"]').or(
        page.locator('text=/audit|activity log/i')
      )
