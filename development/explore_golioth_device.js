...
async function exploreDevice() {
  ...
  console.log('-'.repeat(80))
  const data = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/devices/${DEVICE_ID}/data`
  )
  ...
  const stream = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/stream?deviceId=${DEVICE_ID}`
  )
  ...
  const state = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/devices/${DEVICE_ID}/state`
  )
  ...
  const settings = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/devices/${DEVICE_ID}/settings`
  )
  ...
  const logs = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/devices/${DEVICE_ID}/logs`
  )
  ...
  if (logs.ok && logs.data?.list) {
    ...
    console.log(
      'Sample logs:',
      JSON.stringify(logs.data.list.slice(0, 3), null, 2)
    )
  } else {
  ...
  console.log('-'.repeat(80))
  const rpc = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/devices/${DEVICE_ID}/rpc`
  )
  ...
  const tags = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/devices/${DEVICE_ID}/tags`
  )
  ...
  console.log('-'.repeat(80))
  const creds = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/devices/${DEVICE_ID}/credentials`
  )
  console.log(
    'Endpoint: GET /projects/{projectId}/devices/{deviceId}/credentials'
  )
  ...
  console.log('-'.repeat(80))
  const cohort = await goliothRequest(
    `/projects/${GOLIOTH_PROJECT_ID}/devices/${DEVICE_ID}`
  )
  ...
  if (cohort.ok && cohort.data?.data?.cohortId) {
    ...
    const cohortDetails = await goliothRequest(
      `/projects/${GOLIOTH_PROJECT_ID}/cohorts/${cohort.data.data.cohortId}`
    )
  ...
  if (cohort.ok && cohort.data?.data?.metadata?.update) {
    console.log(
      ...
      JSON.stringify(cohort.data.data.metadata.update, null, 2)
    )
  } else {
