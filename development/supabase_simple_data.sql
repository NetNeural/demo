-- NetNeural IoT Dashboard - Simple Sample Data Script
...
-- Clear existing demo data (ignore errors if records don't exist)
...
-- Insert fresh sample locations (use ON CONFLICT to handle duplicates)
...
-- Insert fresh sample sensors (use ON CONFLICT to handle duplicates)
...
-- Insert fresh sample alerts (use ON CONFLICT to handle duplicates)
...
-- Create additional recent readings (last 2 hours) for more granular data
INSERT INTO public.sensor_readings (sensor_id, value, unit, reading_time)
SELECT 
  s.id,
  CASE s.type
...
-- Set up permissions (ignore all errors)
DO $$ 
BEGIN
...
-- Verification query
SELECT 
  '🎉 SUCCESS! IoT Dashboard sample data loaded!' as status,
  (SELECT COUNT(*) FROM public.locations WHERE id::text LIKE '550e8400-e29b-41d4-a716-4466554404%') as locations,
  (SELECT COUNT(*) FROM public.sensors WHERE id::text LIKE '660e8400-e29b-41d4-a716-4466554404%') as sensors,
  (SELECT COUNT(*) FROM public.alerts WHERE id::text LIKE '770e8400-e29b-41d4-a716-4466554404%') as alerts,
  (SELECT COUNT(*) FROM public.sensor_readings WHERE sensor_id::text LIKE '660e8400-e29b-41d4-a716-4466554404%') as readings;
